...
async def _rollback_created_transfers(transfer_ids: List[str]) -> None:
    """Batalkan transfer antar-gudang yang lahir dari SO yang GAGAL di tengah jalan.

    INV-REF-04 — transfer sudah bisa bertaut dua arah ke SO/permintaannya sejak
    lahir. Menghapusnya tanpa menyapu tautan balik meninggalkan dokumen hidup yang
    menunjuk transfer hantu. Satu tempat untuk dua jalur `except` di `create_order`
    supaya sapuannya tidak mungkin lupa disalin ke salah satunya.
    """
    if not transfer_ids:
        return
    from services import doc_refs_service as _refs
    for _tid in transfer_ids:
        await _refs.safe_unlink_all("warehouse_transfer", _tid)
    await db.warehouse_transfers.delete_many({"id": {"$in": transfer_ids}})
...
async def create_order(payload: SalesOrderCreate, request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        await _rollback_created_transfers(created_transfer_ids)
        raise
