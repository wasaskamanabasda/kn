...
from db import db
from services.notification_service import create_notification, create_addressed
from core_utils import rupiah

# FASE N butir 1 — FINANCE akhirnya ikut diberi tahu soal piutang jatuh tempo.
# Sebelum ini penerimanya hanya sales pemegang akun + peran `manager`, padahal yang
# menyiapkan penagihan dan mencatat uang masuk justru **Finance** (peran ke-6 yang
# lahir di FASE E-8): satu-satunya pihak yang tidak pernah diberi tahu adalah pihak
# yang pekerjaannya paling bergantung pada informasi itu.
#
# Alamatnya diturunkan dari WEWENANG, bukan dari daftar nama peran: `ar_receipt.create`
# = "boleh mencatat uang masuk pelanggan" → hari ini admin · manager · finance. Kalau
# besok pemilik memberi wewenang itu ke peran lain dari layar Matriks Izin, penerima
# pengingat piutang ikut berubah tanpa ada baris kode yang diedit. Memakai satu
# panggilan ber-izin (bukan satu untuk manager + satu untuk finance) sekaligus
# menghindari manajer menerima pesan yang SAMA dua kali.
_AR_TEAM_PERMISSION = ("ar_receipt", "create")
    ...
    if pid:
        ...
        for o in orders:
            ...
            note_s = await create_notification(
                ...
                # FASE N (K2): alamat PRIBADI — pemegang akun saja. Menulis
                # `recipient_role="sales"` di sini menyiarkannya ke SEMUA sales.
                recipient_role="", recipient_user=owner.get("id") or None,
                dedupe_scope="day",
            )
            if note_s:
                created += 1
...
async def notify_backorder_ready(order: Dict[str, Any], product_id: str = "",
    ...
    note = await create_notification(
        ...
        # FASE N (K2): hanya sales pemegang akunnya.
        recipient_role="",
...
async def job_ar_due_soon() -> Dict[str, Any]:
    ...
    for r in rows[:MAX_ALERTS_PER_JOB]:
        ...
        note = await create_notification(
            ...
            # FASE N — dulu: `recipient_role="sales"` BERSAMA `recipient_user=<pemegang
            # akun>`. Karena penyaring pembaca memakai OR, kombinasi itu membuat
            # SELURUH sales melihat piutang pelanggan rekannya (bukan menyempitkan,
            # seperti yang disangka). Sekarang: hanya pemegang akun, dan hanya bila
            # akunnya memang tercatat.
            recipient_role="", recipient_user=r.get("assigned_sales_id") or None,
            dedupe_scope="day",
        )
        if note:
            created += 1
        note_m = await create_addressed(
            permission=_AR_TEAM_PERMISSION,
            notif_type="ar_due_soon",
            ref=f"ar_due_team:{r.get('order_id')}:H{offset:+d}",
            title=f"Piutang {when}: {r.get('number', '')}",
            body=body, severity=sev, link="ar-aging", entity_id=r.get("entity_id"),
            dedupe_scope="day",
        )
