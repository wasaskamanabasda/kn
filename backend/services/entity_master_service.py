...
MASTERS: Dict[str, MasterSpec] = {
    ...
    # ── FASE S (2026-08-23) — JENIS SAMPLING: labdip · handfeel · proofing (boleh
    # LEBIH DARI SATU dalam satu permintaan). Dibuat master karena DUA kelakuan yang
    # dulu tertanam di kode kini jadi data di baris ini:
    ...
    #                           sehingga handfeel/proofing kehilangan kolom yang relevan
    # Berlapis (global → badan usaha) karena satu badan usaha boleh punya jenis yang
    # tidak dipakai badan usaha lain (mis. hanya KSC yang mengerjakan proofing).
    # Benih: `domain_registry.SAMPLE_TYPES`; pembacanya satu: `services/master_registry.py`.
    "sample-types": MasterSpec(
        ...
        fields=("code", "name", "applies_to_lines", "seq", "active", "notes",
                # proofing → wajib kode desain (perilaku lama, kini data)
                "requires_design",
                # hasil ukur yang WAJIB diisi saat menutup round jenis ini
                "measurement_fields"),
        sort=(("seq", 1), ("code", 1)),
        hint="Jenis sampling yang bisa diminta ke supplier. Satu permintaan boleh "
             "menempuh beberapa jenis sekaligus (labdip + handfeel), masing-masing "
             "dengan rangkaian round sendiri. `requires_design` memaksa kode desain; "
             "`measurement_fields` menentukan kolom hasil ukur di form setor hasil.",
    ),
    # ── FASE I — ALASAN KELUHAN RETUR ──────────────────────────────────────────
    # Pemilik ingin lembar retur menyebut SEBABNYA. Dijadikan master (bukan enum
    # beku) karena sebab keluhan tumbuh dari lapangan: kode yang hari ini belum ada
    # ("bau", "tepi bergelombang") harus bisa ditambah tanpa programmer — dan
    # laporan "keluhan apa yang paling sering" hanya berarti kalau kodenya stabil.
    # Berlapis global → badan usaha: satu badan usaha boleh punya kode sendiri.
    # Benih: `domain_registry.COMPLAINT_REASONS`; pembacanya satu:
    # `services/master_registry.py`.
    "complaint-reasons": MasterSpec(
        kind="complaint-reasons", collection="complaint_reasons",
        label="Alasan Keluhan Retur",
        key_field="code", name_field="name", id_prefix="creas",
        fields=("code", "name", "applies_to_lines", "seq", "active", "notes",
                # keluhan yang WAJIB dibuktikan lewat inspeksi fisik
                "needs_inspection",
                # wajar diteruskan sebagai klaim ke supplier/makloon
                "claimable"),
        sort=(("seq", 1), ("code", 1)),
        hint="Sebab keluhan yang bisa dipilih saat mencatat retur pelanggan. "
             "`needs_inspection` memaksa retur itu diperiksa fisik (SPK Inspeksi) "
             "sebelum kredit/ganti diberikan; `claimable` menandai keluhan yang "
