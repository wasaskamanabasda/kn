...
async def _notify_requester(req: Dict[str, Any], outcome: str, detail: str) -> None:
    """Beri tahu PEMOHON tentang keputusan atas permintaannya.

    FASE N — dulu baris ini menulis `recipient_role="all"` **dan** `recipient_user`
    sekaligus. Penyaring pembaca memakai OR (`recipient_role ∈ {peran, "all"}` OR
    `recipient_user`), jadi kombinasi itu bukan menyempitkan alamat melainkan
    MENYIARKAN keputusan internal satu orang ke seluruh pengguna. Alamat yang benar
    untuk peristiwa ini hanya satu: pemohonnya sendiri.
    """
    try:
        from services import notification_service as notif
        requester = req.get("requested_by_id") or ""
        if not requester:
            return          # tanpa pemohon tak ada yang berhak diberi tahu — dan
                            # "kirim ke semua" bukan penggantinya (lihat FASE N)
        await notif.create_notification(
            ...
            entity_id=req["entity_id"], recipient_role="",
