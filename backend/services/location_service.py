...
async def putaway_roll(roll_id: str, bin_id: str, actor_name: str = "System") -> Dict[str, Any]:
    ...
    FASE I (keputusan pemilik #5) — **pagar tahanan**: roll yang hasil inspeksinya
    menandai warna (atau handfeel, bila kebijakannya begitu) BEDA dari sample yang
    di-ACC tidak boleh masuk rak sebelum **manajer** melepas tahanannya ber-alasan.
    Pagar dipasang di sini, bukan di layar, karena putaway punya beberapa pintu
    (layar penempatan, pemindaian, dan skrip); satu-satunya tempat yang mustahil
    dilewati adalah fungsi ini. Pesannya MENUNTUN (menyebut nomor SPK inspeksi &
    siapa yang berwenang), sebab "403 tanpa penjelasan" akan berakhir jadi WhatsApp
    ke admin.
    """
    roll = await db.inventory_rolls.find_one({"id": roll_id}, {"_id": 0})
    if not roll:
        raise HTTPException(status_code=404, detail="Roll tidak ditemukan")
    hold = ((roll.get("inspection") or {}).get("hold") or {})
    if hold.get("held"):
        doc_no = (roll.get("inspection") or {}).get("inspection_doc_number", "")
        raise HTTPException(
            status_code=400,
            detail=(f"Roll {roll.get('roll_no', '')} DITAHAN: {hold.get('reason', '')} "
                    f"Barang belum boleh masuk rak. Yang berwenang melepas tahanan ini "
                    f"adalah MANAJER, lewat dokumen inspeksi"
