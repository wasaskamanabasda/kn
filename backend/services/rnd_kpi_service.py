...
# ─── FASE S — pembaca jenis sampling (satu definisi, dipakai KPI & papan SLA) ──
# Rapor & papan eskalasi TIDAK boleh punya salinan logika ini: begitu satu
# permintaan menempuh dua jenis, "jenis sample" pada baris rapor harus berarti
# jenis ROUND-nya (labdip vs handfeel), bukan jenis dokumennya. Salinan yang
# menyimpang akan membuat dua laporan menyebut angka berbeda untuk satu round.
...
def _round_type(sample: Dict[str, Any], rnd: Dict[str, Any]) -> str:
    code = str(rnd.get("type_code") or "").strip().lower()
    if code:
        return code
    kinds = _types_of(sample)
...
async def my_rounds(query: Optional[Dict[str, Any]] = None, *, name: str,
    ...
    rows = await db[COLL].find(dict(query or {}), {
        ...
        "status": 1, "created_by": 1, "rounds": 1,
    }).to_list(5000)
    ...
    for s in rows:
        ...
        for rd in (s.get("rounds") or []):
            ...
            out.append({
                ...
                "title": s.get("title", ""),
                # FASE S — round menyebut JENIS-nya sendiri (`type_code`); daftar
                # jenis dokumen ikut dikirim supaya rapor bisa berkata "labdip &
                # handfeel" tanpa menebak dari dokumen induk.
                "sample_type": _round_type(s, rd),
                "sample_types": _types_of(s),
