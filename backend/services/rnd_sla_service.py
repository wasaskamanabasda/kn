...
async def overdue_rounds(query: Optional[Dict[str, Any]] = None, *,
    ...
    # FASE S — label jenis sampling dibaca dari MASTER (`sample_types`) DI SERVER,
    # bukan dari peta hardcode di layar. Alasannya dua: (1) jenis yang DITAMBAH
    # pemilik harus langsung terbaca di papan eskalasi & notifikasi WhatsApp tanpa
    # menyentuh frontend; (2) papan ini dipakai layar milik peran `designer` yang
    # TIDAK punya izin `rnd.view` — kalau layarnya harus memanggil
    # `/api/rnd/sample-types` sendiri, panelnya akan mati 403 (pelajaran FASE D:
    # "endpoint yang menumpang membuat layar lain dituduh panel mati").
    label_cache: Dict[str, Dict[str, str]] = {}
    out: List[Dict[str, Any]] = []
    rows = await db[COLL].find(flt, {
        "_id": 0, "id": 1, "number": 1, "title": 1, "sample_types": 1, "sample_type": 1,
        "status": 1, "entity_id": 1, "created_by": 1, "customer_id": 1, "rounds": 1,
    }).to_list(5000)
    for s in rows:
        ...
        for rd in (s.get("rounds") or []):
            ...
            if late <= 0:
                continue
            if ent not in label_cache:
                # `sample_types()` (bukan yang aktif saja) — jenis yang SUDAH
                # dinonaktifkan pemilik tetap harus punya nama manusia untuk
                # dokumen lama yang memakainya.
                label_cache[ent] = {str(r.get("value") or ""): str(r.get("label") or "")
                                    for r in await mreg.sample_types(ent)}
            code = kpi._round_type(s, rd)                # noqa: SLF001 — satu definisi
            out.append({
                "sample_id": s.get("id", ""), "number": s.get("number", ""),
                "title": s.get("title", ""),
                # FASE S — jenis ROUND (bukan jenis dokumen): satu permintaan bisa
                # menempuh labdip & handfeel, dan yang terlambat hanya salah satunya.
                "sample_type": code,
                "sample_type_label": label_cache[ent].get(code) or code,
                "sample_types": kpi._types_of(s),        # noqa: SLF001
                "entity_id": ent,
