...
def _act(key: str, label: str, module: str, action: str, domain: str,
         ...
         docs: Optional[List[Tuple[Any, ...]]] = None) -> Dict[str, Any]:
    """Satu jenis kegiatan + di mana jejaknya bisa dibaca.

    `docs` = (koleksi, field pelaku, field nomor dokumen[, penyaring Mongo]).

    Penyaring opsional itu ADA KARENA SATU KOLEKSI BISA LAHIR DARI DUA PINTU
    dengan izin BERBEDA (terukur 2026-08-21): `purchase_requisitions` lahir dari
    layar PR (`purchase_requisition.create`) **dan** dari tombol "Ulangi pesanan"
    di layar Pesanan (`POST /sales-orders/{id}/repeat-restock`, dipagari
    `order.update` — izin yang memang dimiliki sales, `source="so_repeat"`).
    Tanpa penyaring, laporan ini menuduh sales "di luar peran" untuk pekerjaan
    yang justru dirancang untuknya — dan penjaga yang menuduh palsu akan
    diabaikan, lalu berhenti menjaga apa pun.
    """
...
ACTIVITIES: List[Dict[str, Any]] = [
    ...
    _act("pr_create", "Mengajukan pembelian (PR)", "purchase_requisition", "create",
         ...
         docs=[("purchase_requisitions", "created_by", "number",
                {"source": {"$ne": "so_repeat"}})]),
    # Pintu KEDUA koleksi yang sama: tombol "Ulangi pesanan" di layar Pesanan.
    # Izinnya `order.update` (lihat `routers/sales_orders_extra.repeat_restock`),
    # jadi sales yang memakainya BUKAN "di luar peran".
    _act("restock_request", "Meminta barang masuk dari pesanan (ulangi pesanan)",
         "order", "update", "alur_pesanan", audit_actions=["restock_requested"],
         docs=[("purchase_requisitions", "created_by", "number",
                {"source": "so_repeat"})]),
...
async def _collect_from_docs(by_id, by_name) -> Dict[str, Dict[str, Any]]:
    ...
    for a in ACTIVITIES:
        for spec in a["docs"]:
            coll, field, num_field = spec[0], spec[1], spec[2]
            extra = spec[3] if len(spec) > 3 else None
            if coll not in existing:
                continue
            proj = {"_id": 0, field: 1, num_field: 1, "created_at": 1, "updated_at": 1}
            flt: Dict[str, Any] = {field: {"$nin": [None, ""]}}
            if extra:
                flt.update(extra)
            async for row in db[coll].find(flt, proj):
