...
def a_registry_parity():
    ...
    # FASE D — asersi ini DULU mengunci angka & daftar nama:
    #     set(rr.ROLES) == {"admin","manager","sales_admin","finance","sales","warehouse"}
    # Begitu peran ke-7 (`designer`) lahir, POC memerah TANPA ada yang rusak — dan
    # tekanan berikutnya adalah "tambahkan saja namanya", yang berarti daftar peran
    # ditulis untuk KETIGA kalinya (server, layar, dan di sini). Yang benar-benar
    # ingin dijaga E8.1 bukan "ada berapa peran", melainkan **server & layar tidak
    # bercabang**. Itulah yang diperiksa sekarang; jumlahnya hanya dilaporkan.
    ROLES_INTI = {"admin", "manager", "sales_admin", "finance", "sales", "warehouse"}
    kurang = sorted(ROLES_INTI - set(rr.ROLES))
    check(f"registry peran terisi & memuat peran inti ({len(rr.ROLES)} peran terdaftar)",
          not kurang,
          " · ".join(rr.role_ids()) + (f" · KURANG: {kurang}" if kurang else ""))
    ...
    for m in re.finditer(r"^  (\w+):\s*\{(.*?)^  \},", js, re.S | re.M):
        ...
        # FASE D: dulu ada daftar-putih nama peran di sini — sumber KEEMPAT, dan ia
        # membuat peran ke-7 tak terlihat oleh POC (jadi percabangan server↔layar
        # justru TIDAK terperiksa untuk peran yang paling baru). Sekarang SEMUA entri
        # registry layar dibaca lalu dibandingkan apa adanya dengan server.
    ...
    hanya_be = sorted(set(rr.ROLES) - set(fe))
    hanya_fe = sorted(set(fe) - set(rr.ROLES))
    check("layar mengenal peran yang SAMA dengan server (nol percabangan)",
          set(fe) == set(rr.ROLES),
          f"hanya server: {hanya_be} · hanya layar: {hanya_fe}" if (hanya_be or hanya_fe)
          else f"{len(fe)} peran identik: " + " · ".join(sorted(fe)))
    ...
    check("label · peringkat · lintas-entitas · urutan IDENTIK server↔layar",
          not beda, "bercabang: " + ", ".join(beda) if beda
          else f"{len(rr.ROLES)}/{len(rr.ROLES)} sama")
    ...
    check("gate check_nav_map PASS (beranda tiap peran ter-reach dari menunya)",
...
def b_identity(adm, sa, fin):
    ...
    # FASE D: dulu `len(daftar) == 6`. Angka itu bukan invariannya — invariannya
    # adalah "server mengirim SEMUA peran yang ada di registrynya".
    check(f"GET /api/roles mengirim seluruh {len(rr.ROLES)} peran + penjelasannya",
          set(daftar) == set(rr.ROLES),
          f"HTTP {r.status_code} · {' · '.join(sorted(daftar))}"
          + (f" · KURANG: {sorted(set(rr.ROLES) - set(daftar))}"
             if set(rr.ROLES) - set(daftar) else ""))
