...
RECOUNT: Dict[str, tuple] = {
    ...
    # FASE D (2026-08-20) — antrean PERMINTAAN DESAIN (`design_requests.status ==
    # "delivered"`: artwork sudah diserahkan, atasan belum memutuskan). Ditulis ULANG
    # di sini karena POC ini adalah OPINI KETIGA (backend `QUEUES` = pertama,
    # `verify_home_kpi.EXPECT` = kedua). Sengaja BUKAN kembaran `design_gallery`:
    # galeri memutuskan ARTWORK-nya, permintaan desain memutuskan PEKERJAAN-nya —
    # dan G6 di bawah membuktikan tidak ada dokumen yang terhitung di dua antrean.
    "design_request": ("design_requests", {"status": "delivered"}),
    # FASE I (2026-08-23) — antrean BARANG DITAHAN. Ditulis ULANG di sini dengan
    # sengaja (bukan mengimpor `QUEUES`): POC ini adalah **opini kedua**, jadi kalau
    # definisi backend kelak bergeser, kedua implementasi tidak akan setuju lagi dan
    # POC MEMERAH — itu gunanya. Yang menunggu bukan "inspeksi belum selesai" (itu
    # pekerjaan) melainkan tahanan yang hanya MANAJER boleh lepas.
