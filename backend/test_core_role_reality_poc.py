...
POC_PR_IDS = []                   # PR sintetis R6b — WAJIB nol di akhir (INV-GATE-01)
...
async def _insert_pr(doc):
    """PR sintetis untuk R6b (arah sebaliknya) — dihapus lagi di pemeriksaan yang sama."""
    cl = _mongo()
    try:
        db = cl[os.environ["DB_NAME"]]
        await db.purchase_requisitions.insert_one(dict(doc))
        POC_PR_IDS.append(doc["id"])
    finally:
        cl.close()


async def _delete_pr(ids):
    cl = _mongo()
    try:
        db = cl[os.environ["DB_NAME"]]
        await db.purchase_requisitions.delete_many({"id": {"$in": list(ids)}})
    finally:
        cl.close()
...
async def _cleanup(before_ids, role_restore):
    ...
    try:
        ...
        # PR sintetis R6b: sudah dihapus di tempatnya, tetapi pengaman kedua di sini
        # supaya POC yang mati di tengah tidak meninggalkan permintaan pembelian hantu.
        if POC_PR_IDS:
            await db.purchase_requisitions.delete_many({"id": {"$in": list(POC_PR_IDS)}})
...
# ─── R6b · SATU koleksi, DUA pintu, izin berbeda (anti tuduh-palsu) ──────────
def c_two_doors(adm):
    """Bukti dua arah untuk kelas tuduhan palsu yang NYATA (terukur 2026-08-21).

    `purchase_requisitions` lahir dari DUA pintu: layar PR (`purchase_requisition.create`)
    dan tombol **"Ulangi pesanan"** di layar Pesanan (`order.update`, `source="so_repeat"`).
    Data demo FASE P memakai pintu kedua sebagai SALES — dan laporan versi lama
    menyebutnya "di luar peran". Yang palsu bukan datanya, melainkan pemetaannya.
    """
    head("LANGKAH 3b — satu koleksi, dua pintu: PR dari 'Ulangi pesanan' BUKAN di luar peran (R6b)")
    ayu = row_of(report(adm), SALES)
    ev = {e["permission"]: e for e in ayu.get("evidence", [])}
    check("R6b · data demo memang punya PR `so_repeat` milik sales (bukan hijau tapi hampa)",
          "order.update" in ev and ev["order.update"]["count"] >= 1,
          f"{ev.get('order.update', {}).get('count', 0)} PR · "
          f"{ev.get('order.update', {}).get('samples')}")
    check("R6b · PR `so_repeat` dipetakan ke izin PINTUNYA (`order.update`), bukan `purchase_requisition.create`",
          "purchase_requisition.create" not in ev,
          " · ".join(sorted(ev)))
    check("R6b · karena itu sales tetap `sesuai` di data demo (nol tuduhan palsu)",
          ayu.get("verdict") == "sesuai", f"→ {ayu.get('verdict')}")

    # Arah sebaliknya: PR yang lahir dari LAYAR PR (bukan tombol pesanan) HARUS
    # tetap dituduh — kalau tidak, penyaring di atas berubah jadi lubang pembebasan.
    pr_id = "pr_poc_rr_manual"
    asyncio.run(_insert_pr({
        "id": pr_id, "number": "PR-POC-RR", "entity_id": ENT_A, "source": "manual",
        "created_by": "Ayu Permatasari", "status": "draft", "items": [],
        "created_at": "2026-08-05T02:00:00+00:00"}))
    ayu2 = row_of(report(adm), SALES)
    ev2 = {e["permission"]: e for e in ayu2.get("evidence", [])}
    check("R6b · PR dari LAYAR PR (source=manual) TETAP dituduh di luar peran (penyaring bukan lubang)",
          ayu2.get("verdict") == "di_luar_peran"
          and ev2.get("purchase_requisition.create", {}).get("beyond_current_role") is True,
          f"→ {ayu2.get('verdict')} · izin={sorted(ev2)}")
    asyncio.run(_delete_pr([pr_id]))
    check("R6b · keadaan pulih: sales `sesuai` lagi sesudah PR uji dihapus",
          row_of(report(adm), SALES).get("verdict") == "sesuai")
