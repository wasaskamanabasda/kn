...
def main() -> int:  # noqa: C901 — POC linear supaya mudah dibaca sebagai bukti
    ...
    audit_removed = (db.audit_logs.delete_many({"id": {"$in": list(new_audit)}}).deleted_count
                     if new_audit else 0)
    # INV-REF-04 — SPK uji sudah bertaut DUA ARAH ke kontrak supplier seed
    # (`fulfills`/`fulfilled_by`). Menghapus SPK-nya saja meninggalkan kontrak demo
    # yang MASIH HIDUP menunjuk SPK yang tak ada: terukur `KSC/SCT-00004` dan
    # `KSC/SCT-00008` menawarkan 3 tautan menuju 404, dan itu menjatuhkan
    # `audit_doc_refs --strict` serta POC P-0 yang memanggilnya.
    swept = sweep_ghost_refs()
    ok(sweep_ghost_refs(verbose=False) == 0,
       f"nol tautan hantu sesudah SPK uji dihapus (INV-REF-04 · {swept} disapu)")
