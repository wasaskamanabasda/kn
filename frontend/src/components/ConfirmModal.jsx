 ...
 *  - reasonMinLength: panjang MINIMUM alasan (0/1 = cukup tidak kosong). Ambangnya
 *    datang dari SERVER (mis. `/api/inspections/meta` → `policy.min_reason_length`),
 *    bukan ditebak layar. Alasannya terukur 2026-08-23: dialog ini dulu hanya menolak
 *    isian KOSONG, sehingga alasan "pendek" lolos di layar lalu dijawab **400** oleh
 *    server yang menuntut minimal 15 huruf — pengguna melihat galat merah untuk aturan
 *    yang seharusnya sudah kelihatan sebelum tombolnya bisa ditekan.
...
export default function ConfirmModal({
  ...
  const blocked = busy || (withReason && reasonRequired
    && reason.trim().length < Math.max(1, Number(reasonMinLength) || 0));
      ...
      <div className="modal-card small" role="dialog" aria-modal="true" aria-label={title}>
        ...
        {withReason && (
          <div className="grid gap-1.5 mt-2">
            ...
            {/* Petunjuk HIDUP: menyebut ambangnya dan berapa huruf lagi yang kurang.
                Tombol yang mati tanpa penjelasan terbaca sebagai aplikasi rusak. */}
            {withReason && reasonRequired && Number(reasonMinLength) > 1 && (
              <span data-testid={`${testId}-reason-hint`}
                className={`text-[10.5px] leading-snug ${
                  reason.trim().length >= Number(reasonMinLength)
                    ? "text-[#1E8E3E]" : "text-[#B45309]"}`}>
                {reason.trim().length >= Number(reasonMinLength)
                  ? "Alasan sudah memadai."
                  : `Alasan wajib minimal ${Number(reasonMinLength)} huruf — `
                    + `kurang ${Number(reasonMinLength) - reason.trim().length} huruf. `
                    + "Alasan yang tidak bisa dipakai orang lain sama saja dengan kolom kosong."}
              </span>
            )}
          </div>
        )}
