...
const TYPE_LABEL = {
  ...
  // FASE N — peristiwa yang alamatnya diturunkan dari WEWENANG (bukan nama peran).
  special_order_approval: "PO custom menunggu keputusan",
                ...
                >
                  ...
                  <div className="notif-item-body">
                    <div className="notif-item-title">
                      ...
                      {/* FASE N — ALAMAT ikut terlihat. Sebelum ini kotak notifikasi
                          tidak pernah memberi tahu MENGAPA sebuah pesan sampai ke
                          kita, padahal 11 pesan dikirim ke "semua orang" sehingga
                          Finance menerima 9 pesan stok kain. Sesudah alamatnya
                          diturunkan dari wewenang, pita ini yang membuat perbaikannya
                          bisa DILIHAT pengguna (dan salah alamat bisa dilaporkan). */}
                      {n.recipient_user ? (
                        <span className="notif-addr-tag notif-addr-me"
                              data-testid={`notif-addr-me-${n.id}`}
                              title="Ditujukan khusus kepada Anda — bukan siaran ke semua peran">
                          <UserCheck size={9} /> Untuk Anda
                        </span>
                      ) : (n.recipient_role && n.recipient_role !== "all") ? (
                        <span className="notif-addr-tag notif-addr-role"
                              data-testid={`notif-addr-role-${n.id}`}
                              title={`Ditujukan ke peran ${roleLabel(n.recipient_role)}`}>
                          {roleLabel(n.recipient_role)}
                        </span>
                      ) : (
                        <span className="notif-addr-tag notif-addr-all"
                              data-testid={`notif-addr-all-${n.id}`}
                              title="Pengumuman umum — dikirim ke semua peran">
                          Umum
                        </span>
                      )}
                    </div>
