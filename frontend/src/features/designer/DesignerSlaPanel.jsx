 ...
 *
 * FASE S — nama jenis sampling datang **dari server** (`sample_type_label`, dibaca
 * `rnd_sla_service` dari master `sample_types`). Sebelumnya panel ini menerjemahkan
 * kode lewat peta hardcode `SAMPLE_TYPE_LABEL` di frontend; itu benar hanya untuk
 * tiga jenis baku — jenis yang DITAMBAH pemilik lewat master (mis. `sanforize`)
 * muncul di papan sebagai kode teknis, dan `bulk_sample` yang sudah dinonaktifkan
 * tetap membawa label lama yang dibekukan di layar. Label dari server juga
 * menghindari panel ini memanggil `/api/rnd/sample-types` sendiri: peran `designer`
 * tidak punya izin `rnd.view`, jadi panggilan itu akan mematikan panelnya (403).
 *
 * Catatan jujur: field `sample_type` pada baris papan ini BUKAN field dokumen yang
 * dihapus migrasi FASE S — ia dihitung server sebagai jenis ROUND (`type_code`),
 * jadi papan lama tidak pernah kosong; yang salah hanya kosakatanya.
 */
                  ...
                  <div key={r.round_id} data-testid={`designer-sla-row-${r.round_id}`}
                    ...
                    <span className="min-w-0 flex-1 truncate">
                      ...
                      <span className="text-[#9A9BA3]">
                        {" · "}{r.sample_type_label || r.sample_type || "—"}
