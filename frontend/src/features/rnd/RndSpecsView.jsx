...
import { typeLabel } from "./sampleTypeMeta";
...
export default function RndSpecsView({ currentUser, selectedEntity, focus, onFocusConsumed }) {
  ...
  const [types, setTypes] = useState([]);   // FASE S — jenis sampling dari MASTER
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // FASE S — jenis sampling dari MASTER (`/api/rnd/meta`). Dipakai kolom
      // "Rencana sample" dan diteruskan ke form + panel rincian supaya ketiganya
      // memakai satu kosakata; jenis yang ditambah pemilik langsung terbaca.
      setTypes(Array.isArray(meta?.sample_types) ? meta.sample_types : []);
      setError("");
    } catch (e) {
  ...
  return (
    <div data-testid="rnd-specs-view">
            ...
            {filtered.map((s) => {
              ...
              return (
                <div key={s.id} data-testid={`rnd-spec-row-${s.id}`}
                  ...
                  <span className="text-[11px]" data-testid={`rnd-spec-type-${s.id}`}>
                    {typeLabel(s.sample_type_hint, types)}
                  </span>
                  <div className="min-w-0">
      ...
      {showForm && (
        <SpecFormModal selectedEntity={selectedEntity} types={types}
      ...
      {openId && (
        <DetailModal onClose={() => setOpenId("")}
          ...
          <SpecDetailPanel specId={openId} currentUser={currentUser} types={types}
