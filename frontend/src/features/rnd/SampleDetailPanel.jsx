/**
 * SampleDetailPanel (FASE F · PS-18/PS-19, diperluas FASE S) — pusat kerja SATU
 * permintaan sample.
 *
 * Urutan kerja nyata:
 *   1. kirim permintaan ke ≥1 supplier × ≥1 JENIS (bisa dibandingkan per jenis),
 *   2. unggah bukti + setor hasil per round (`rnd 1 → n`, hasil ukur dari master),
 *   3. nilai hasil (acc / revisi / tolak) + skor → jejak QC sample tercatat,
 *   4. buka round berikutnya bila revisi (kuota dihitung PER JENIS),
 ...
 *   7. **FASE S** tandai "Sample Jadi" lalu "Kirim" (tujuan WAJIB) — user story S.F-4.
 */
...
import { errMsg, ROUND_RESULT_META, SAMPLE_STATUS_META } from "./rndMeta";
...
export default function SampleDetailPanel({ sampleId, currentUser, types: typesProp,
  ...
  const [modal, setModal] = useState(null);   // { kind, round, participant, typeCode }
  ...
  useEffect(() => {
    rndMeta().then((m) => {
      ...
      setDeliverTargets(m?.deliver_targets || []);
      if ((m?.sample_types || []).length) setTypes(m.sample_types);
    }).catch(() => { /* meta opsional — layar tetap berfungsi dengan label cadangan */ });
  }, []);
  ...
  return (
    <div data-testid="sample-detail-panel"
      ...
      <div className="flex max-h-[94vh] w-full max-w-[1000px] flex-col overflow-hidden rounded-xl bg-white shadow-2xl"
        ...
        <div className="flex items-center justify-between gap-2 border-b border-[#EFF0F2] px-4 py-3">
          <div className="min-w-0">
            <h2 className="flex flex-wrap items-center gap-2 text-[15px] font-bold">
              ...
              {/* FASE S — satu lencana PER JENIS (dulu satu label tunggal). */}
              <span className="flex flex-wrap gap-1" data-testid="sample-detail-types">
                {codes.map((c) => (
                  <span key={c} className="status-pill pill-muted"
                    data-testid={`sample-detail-type-${c}`}>{typeLabel(c, types)}</span>
                ))}
              </span>
            </h2>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          ...
          {/* FASE S · user story S.F-2 & S.F-4 — tautan pesanan + penanda jadi/kirim */}
          <div className="grid gap-2 md:grid-cols-3">
            <Box label="Untuk pesanan" testId="sample-detail-order"
              value={sample?.so_id
                ? `${sample.so_number || "tertaut"} — lihat Jejak dokumen`
                : "Bukan dari pesanan tertentu"} />
            <Box label="Sample jadi" testId="sample-detail-finished"
              tone={sample?.finished_at ? "#1B7F4B" : "#8E8E93"}
              value={sample?.finished_at ? String(sample.finished_at).slice(0, 10) : "Belum"} />
            <Box label="Dikirim" testId="sample-detail-delivered"
              tone={sample?.delivered_at ? "#1B7F4B" : "#8E8E93"}
              value={sample?.delivered_at
                ? `${String(sample.delivered_at).slice(0, 10)} → `
                  + `${deliverLabel(sample.delivered_to, deliverTargets)}`
                  + `${sample.delivered_to_name ? ` (${sample.delivered_to_name})` : ""}`
                : "Belum"} />
          </div>
          ...
          {/* Perbandingan supplier — FASE S: dipecah PER JENIS supaya labdip tidak
              dibandingkan dengan handfeel (skornya beda arti). */}
          {(sample?.participants || []).length > 1 && (
            <div className="rounded-lg border border-[#EFF0F2]" data-testid="sample-compare">
              <p className="border-b border-[#EFF0F2] bg-[#FAFBFC] px-3 py-1.5 text-[10.5px] font-bold uppercase text-[#8E8E93]">
                Perbandingan supplier per jenis
              </p>
              {codes.map((tc) => (
                <div key={tc} data-testid={`sample-compare-type-${tc}`}>
                  <p className="bg-white px-3 py-1 text-[10.5px] font-bold text-[#0058CC]">
                    {typeLabel(tc, types)}
                  </p>
                  <div className="grid grid-cols-[1.4fr_80px_90px_1fr_110px] px-3 py-1 text-[9.5px] font-bold uppercase text-[#8E8E93]">
                  ...
                  <div className="divide-y divide-[#F4F5F7]">
                    {(sample.participants || []).map((p) => {
                      const rs = (sample.rounds || []).filter(
                        (r) => r.supplier_id === p.supplier_id
                          && roundTypeOf(r, sample) === tc);
                      if (rs.length === 0) return null;
                      const last = rs[rs.length - 1] || {};
                      const rm = ROUND_RESULT_META[last.result || ""] || ROUND_RESULT_META[""];
                      const cost = rs.reduce((a, r) => a + Number(r.cost || 0), 0);
                      const best = rs.reduce((a, r) => (r.score != null
                        ? Math.max(a ?? 0, Number(r.score)) : a), null);
                      return (
                        <div key={`${tc}-${p.supplier_id}`}
                          data-testid={`sample-compare-row-${tc}-${p.supplier_id}`}
                          className="grid grid-cols-[1.4fr_80px_90px_1fr_110px] items-center px-3 py-1.5 text-[11.5px]">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 border-t border-[#EFF0F2] px-4 py-3">
          {cancelOpen ? (
            ...
            <>
              ...
              {/* FASE S · user story S.F-4 — dua langkah pelaksanaan berurutan. */}
              {!cancelled && canSubmit && !sample?.finished_at && (
                <button className="secondary-button" disabled={busy || !hasAcc}
                  data-testid="sample-finish-button"
                  title={hasAcc ? "Tandai sample sudah jadi"
                    : "Belum ada round yang ACC — nilai dulu hasil sample-nya"}
                  onClick={() => setModal({ kind: "finish" })}>
                  <CheckCircle2 size={13} /> Sample Jadi
                </button>
              )}
              {!cancelled && canSubmit && sample?.finished_at && !sample?.delivered_at && (
                <button className="secondary-button" disabled={busy}
                  data-testid="sample-deliver-button"
                  title="Catat sample sudah dikirim (tujuan wajib)"
                  onClick={() => setModal({ kind: "deliver" })}>
      ...
      {/* ── Modal turunan ────────────────────────────── */}
      {modal?.kind === "send" && (
        <SampleSendModal mode="send" sample={sample} policy={policy} types={types} busy={busy}
      ...
      {modal?.kind === "finish" && (
        <SampleFinishModal mode="finish" sample={sample} busy={busy}
          onClose={() => setModal(null)}
          onConfirm={(body) => act(() => finishSample(sample.id, body),
            "Sample ditandai JADI — tanggalnya tercatat.")} />
      )}
      {modal?.kind === "deliver" && (
        <SampleFinishModal mode="deliver" sample={sample} targets={deliverTargets} busy={busy}
          onClose={() => setModal(null)}
          onConfirm={(body) => act(() => deliverSample(sample.id, body),
            "Pengiriman sample tercatat beserta tujuannya.")} />
      )}
    </div>
  );
}
