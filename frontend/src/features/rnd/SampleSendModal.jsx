/**
 * SampleSendModal (FASE F · PS-18, diperluas FASE S) — dua peran, satu permukaan:
 *  - mode "send"  : kirim permintaan ke **beberapa supplier × beberapa JENIS**
 *                   sekaligus. Round 1 dibuka untuk tiap kombinasi, sehingga hasil
 *                   bisa dibandingkan per jenis sebelum harga dikunci.
 *  - mode "round" : buka **round berikutnya** untuk satu supplier pada SATU JENIS
 *                   setelah hasil `revisi`. Melewati batas `rnd.max_rounds` wajib
 *                   ALASAN tertulis (hanya manager/admin) — server yang menegakkan.
 *                   Kuota dihitung per (supplier × jenis): perbaikan warna tidak
...
// FASE D — daftar supplier hidup DI SINI, bukan di `rndApi.js`. Modul bersama itu
// diimpor juga oleh layar Desain & Pattern / Galeri Desain yang dimiliki peran
// `designer` (tanpa `supplier.view`), sehingga `/suppliers` yang menumpang di sana
// membuat `audit_sales_roles_ux` melaporkan panel mati pada layar yang sebenarnya
// tidak pernah memanggilnya. Modal INI-lah satu-satunya pemakainya.
const listSuppliers = (params) =>
  axios.get(`${API}/suppliers`, { params }).then((r) => r.data);

export default function SampleSendModal({ mode = "send", sample, participant, typeCode,
  types, policy, onClose, onConfirm, busy }) {
  ...
  // "Sudah diundang" kini per (supplier × jenis): supplier yang sudah mengerjakan
  // labdip TETAP boleh diberi handfeel. Sebelum FASE S pemeriksaannya hanya melihat
  // supplier, sehingga pekerjaan sah itu ditolak dengan kalimat "semua supplier
  // sudah diundang".
  const covered = useMemo(() => {
    const set = new Set();
    (sample?.rounds || []).forEach((r) => {
      set.add(`${r.supplier_id}|${roundTypeOf(r, sample)}`);
    });
    return set;
  }, [sample]);
  ...
  const toggleType = (c) => setPickedTypes((p) =>
    p.includes(c) ? p.filter((x) => x !== c) : [...p, c]);

  const roundsOfPartner = (sample?.rounds || [])
    .filter((r) => r.supplier_id === participant?.supplier_id
      && roundTypeOf(r, sample) === typeCode);
  const nextNo = roundsOfPartner.length + 1;
  const overLimit = nextNo > Number(policy?.max_rounds || 3);
  // Berapa round baru yang akan lahir — ditampilkan supaya pengguna tahu persis
  // akibat satu tombol (kombinasi yang sudah punya round tidak dibuat ulang).
  const newRounds = picked.reduce((acc, sid) => acc
    + pickedTypes.filter((t) => !covered.has(`${sid}|${t}`)).length, 0);
      ...
      <div className="flex max-h-[92vh] w-full max-w-[620px] flex-col overflow-hidden rounded-xl bg-white shadow-2xl"
        ...
        <div className="flex items-center justify-between border-b border-[#EFF0F2] px-4 py-3">
          <h2 className="flex items-center gap-2 text-[15px] font-bold">
            ...
            {isSend ? "Kirim Permintaan ke Supplier"
              : `Buka round ${nextNo} · ${typeLabel(typeCode, types)} — `
                + `${participant?.supplier_name || ""}`}
          </h2>
          ...
          {isSend ? (
            <>
              <div className="rounded-lg bg-[#F2F7FF] px-3 py-2 text-[11.5px] text-[#004099]">
                Pilih <b>lebih dari satu supplier</b> bila ingin membandingkan hasil, dan
                <b> jenis mana saja</b> yang dikirim sekarang. Setiap kombinasi
                (supplier × jenis) mendapat <b>round 1</b> sendiri dengan tenggat yang sama.
              </div>

              <div>
                <p className="mb-1 text-[10.5px] font-semibold text-[#6B6B73]">
                  Jenis yang dikirim sekarang
                </p>
                <div className="flex flex-wrap gap-1.5" data-testid="sample-send-types">
                  {kinds.map((c) => {
                    const on = pickedTypes.includes(c);
                    return (
                      <button key={c} type="button" onClick={() => toggleType(c)}
                        data-testid={`sample-send-type-${c}`}
              ...
              <div className="max-h-[240px] divide-y divide-[#F4F5F7] overflow-y-auto rounded-lg border border-[#EFF0F2]"
                ...
                {filtered.map((s) => {
                  const done = pickedTypes.length > 0
                    && pickedTypes.every((t) => covered.has(`${s.id}|${t}`));
                  const on = picked.includes(s.id);
                  const partial = pickedTypes.filter((t) => covered.has(`${s.id}|${t}`)).length;
                  return (
                    <button key={s.id} type="button" disabled={done}
                      data-testid={`sample-send-supplier-${s.id}`}
                      onClick={() => toggle(s.id)}
                      className={`flex w-full items-center justify-between px-3 py-2 text-left ${done
                      ...
                      <span className="min-w-0">
                        ...
                        <span className="block truncate text-[10.5px] text-[#6B6B73]">
                          ...
                          {!done && partial > 0
                            ? ` · ${partial} jenis sudah berjalan` : ""}
                        </span>
                      </span>
                      <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-bold ${done
              ...
              <p className="text-[11px] text-[#6B6B73]" data-testid="sample-send-count">
                {picked.length} supplier × {pickedTypes.length} jenis → <b>{newRounds} round
                baru</b> akan dibuka.
              </p>
            </>
          ) : (
            <div className={`rounded-lg px-3 py-2 text-[11.5px] ${overLimit
              ...
              {overLimit
                ? `Round ${nextNo} jenis ${typeLabel(typeCode, types)} MELEWATI batas `
                  + `${policy?.max_rounds || 3} iterasi. Hanya manager/admin yang boleh `
                  + "membukanya, dan alasan tertulis WAJIB (batas bisa diubah di Pusat "
                  + "Pengaturan → R&D & Desain)."
                : `Round ${nextNo} dari batas ${policy?.max_rounds || 3} iterasi untuk `
                  + `jenis ${typeLabel(typeCode, types)}. Kuota dihitung PER JENIS — `
                  + "perbaikan warna tidak menghabiskan kuota perbaikan handfeel."}
            </div>
          )}
        ...
        <div className="flex items-center justify-end gap-2 border-t border-[#EFF0F2] px-4 py-3">
          ...
          <button className="primary-button" data-testid="sample-send-confirm"
            disabled={busy || (isSend
              ? (picked.length === 0 || pickedTypes.length === 0)
              : (overLimit && !reason.trim()))}
            onClick={() => (isSend
              ? onConfirm({ supplier_ids: picked, type_codes: pickedTypes,
                due_date: due, note })
              : onConfirm({ supplier_id: participant?.supplier_id, type_code: typeCode,
                due_date: due, note, reason }))}>
            <Send size={13} /> {busy ? "Memproses…"
              : isSend ? `Kirim · ${newRounds} round` : `Buka round ${nextNo}`}
          </button>
        </div>
      </div>
    </div>
