...
export const COLUMNS = {
  ...
  // ── FASE S — JENIS SAMPLING. Kolom dipilih supaya pemilik bisa menjawab TIGA
  // pertanyaan langsung dari tabel, tanpa membuka baris: jenis ini menuntut kode
  // desain atau tidak (`requires_design` — dulu tertanam sebagai `if == proofing`
  // di dalam kode), hasil ukur apa yang WAJIB diisi petugas saat menutup round
  // (`measurement_fields` — dulu 5 field TETAP di skema, sehingga handfeel &
  // proofing kehilangan kolom yang relevan), dan untuk lini apa. Kalau tiga hal itu
  // tak terlihat di sini, pemilik baru tahu akibatnya saat petugasnya tertahan di
  // form setor hasil.
  "sample-types": [
    { key: "code", label: "Kode", mono: true },
    { key: "name", label: "Nama jenis" },
    { key: "requires_design", label: "Wajib kode desain?",
      labels: { true: "Ya — harus ada desain", false: "Tidak" } },
    { key: "measurement_fields", label: "Hasil ukur wajib", list: true,
      empty: "tidak ada" },
    { key: "applies_to_lines", label: "Lini", list: true, empty: "semua lini" },
    { key: "seq", label: "Urut", align: "right" },
  ],
  // FASE I — ALASAN KELUHAN RETUR. Dua kolom keputusan ditampilkan dengan sengaja:
  // pemilik perlu melihat sekali pandang keluhan mana yang MEMAKSA pemeriksaan fisik
  // dan mana yang wajar diklaim ke supplier. Tanpa itu, kedua aturan hanya terasa
  // saat petugas retur tertahan atau klaim ditolak.
  "complaint-reasons": [
    { key: "code", label: "Kode", mono: true },
    { key: "name", label: "Alasan keluhan" },
    { key: "needs_inspection", label: "Wajib diperiksa fisik?",
      labels: { true: "Ya — harus ada SPK inspeksi", false: "Tidak" } },
    { key: "claimable", label: "Bisa diklaim ke supplier?",
      labels: { true: "Ya", false: "Tidak" } },
    { key: "applies_to_lines", label: "Lini", list: true, empty: "semua lini" },
    { key: "seq", label: "Urut", align: "right" },
  ],
};
...
export const CREATE_FIELDS = {
  ...
  // ── FASE S — menambah jenis sampling baru (mis. "Wash test") = mengisi form ini,
  // bukan menunggu programmer. Dua field di bawah adalah ATURAN yang dulu hidup di
  // dalam kode: `requires_design` (dulu `if stype == "proofing"`) dan
  // `measurement_fields` (dulu 5 field tetap di skema). Bawaan sengaja dipilih yang
  // PALING AMAN: tanpa kewajiban desain, dan tanpa hasil ukur wajib — jenis baru
  // tidak boleh langsung menahan petugas di form setor hasil sebelum pemiliknya
  // memutuskan apa yang mau diukur.
  "sample-types": [
    { key: "code", label: "Kode jenis", required: true, placeholder: "mis. wash_test",
      hint: "Huruf kecil tanpa spasi. Kode ini yang tersimpan di permintaan sample "
            + "dan di tiap round (`type_code`)." },
    { key: "name", label: "Nama jenis", required: true,
      placeholder: "Wash test (uji cuci)" },
    { key: "requires_design", label: "Wajib merujuk kode desain?", type: "checkbox",
      default: false,
      hint: "Centang untuk jenis yang mustahil dikerjakan tanpa artwork (mis. "
            + "proofing printing). Server akan MENOLAK permintaan tanpa desain." },
    { key: "measurement_fields", label: "Hasil ukur wajib", type: "list",
      placeholder: "delta_e, colorfastness_wash",
      hint: "Kolom yang WAJIB diisi saat menutup round jenis ini. Pilihan yang "
            + "dikenal: delta_e · colorfastness_wash · colorfastness_rub · "
            + "gsm_actual · lebar · shrinkage_pct · handfeel_score · repeat_cm · "
            + "register_mm. Nama di luar daftar itu akan memerahkan gate "
            + "INV-SAMPLE-01 (kolom tanpa label & tanpa batas wajar)." },
    { key: "applies_to_lines", label: "Berlaku untuk lini", type: "list",
      placeholder: "printing", hint: "Kosong = berlaku untuk SEMUA lini." },
    { key: "seq", label: "Urutan tampil", type: "number", default: 100 },
    { key: "notes", label: "Catatan" },
  ],
  // FASE I — alasan keluhan retur. Bawaan `needs_inspection` SENGAJA tidak dicentang:
  // kode baru tidak boleh langsung memaksa setiap retur menunggu pemeriksaan fisik
  // sebelum pemiliknya sadar memilih itu.
  "complaint-reasons": [
    { key: "code", label: "Kode alasan", required: true, placeholder: "mis. bau_apek",
      hint: "Huruf kecil tanpa spasi. Kode ini yang tersimpan di dokumen retur "
            + "(`complaint_code`) dan dipakai laporan “keluhan tersering”." },
    { key: "name", label: "Alasan keluhan", required: true,
      placeholder: "Bau apek / lembap" },
    { key: "needs_inspection", label: "Wajib dibuktikan lewat inspeksi fisik?",
      type: "checkbox", default: false,
      hint: "Centang untuk keluhan yang bisa diperdebatkan (warna, cacat, kurang "
            + "meteran). Retur ber-alasan ini menunggu SPK Inspeksi selesai sebelum "
            + "kredit/ganti diberikan." },
    { key: "claimable", label: "Wajar diklaim ke supplier?", type: "checkbox",
      default: false,
      hint: "Centang bila sebabnya biasanya berasal dari supplier/makloon. Keluhan "
            + "administrasi (mis. salah kirim) sebaiknya TIDAK dicentang." },
    { key: "applies_to_lines", label: "Berlaku untuk lini", type: "list",
      placeholder: "printing", hint: "Kosong = berlaku untuk SEMUA lini." },
    { key: "seq", label: "Urutan tampil", type: "number", default: 100 },
    { key: "notes", label: "Catatan" },
