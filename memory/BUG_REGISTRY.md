...
## REGISTRY — Sesi 2026-08-24 (lanjutan · penutupan 1 FAIL warisan + utang FASE N)

### KN-N-RESIDU-AUDIT — POC FASE N menumbuhkan `audit_logs` +2 setiap gate
- Tanggal       : 2026-08-24
- Modul         : backend/test_core_notifikasi_alamat_poc.py
- Gejala        : `gate.sh --full` MERAH di `INV-GATE-01`: `audit_logs 103 -> 105 (+2)`.
                  Dua baris itu `login` (sesi POC) + `special_order_created` (PO custom uji);
                  POC memulihkan `notifications` & menghapus dokumen ujinya, tetapi jejaknya
                  tidak pernah ikut dibersihkan.
- Cara ditemukan: bukan dengan membaca — `scripts/_bisect_residu_audit.sh` menjalankan
                  KE-53 gate runtime satu per satu dan mengukur selisih `audit_logs`
                  sesudah masing-masing. Hanya satu yang bergerak.
- Perbaikan     : sidik jari himpunan ID `audit_logs` + `sessions` direkam SEBELUM
                  permintaan pertama (jadi jejak login ikut di dalam jendela), lalu
                  dibuang lewat SELISIH HIMPUNAN di blok `finally` — pola yang sama
                  dengan POC FASE S, bukan berdasar waktu atau nama aksi.
- Invariant     : INV-GATE-01
- Gate penangkap: `scripts/gate_residue.py --check` + 2 pemeriksaan baru di POC N sendiri
- Severity      : P1 (satu-satunya FAIL yang menahan gate)
- Status        : FIXED (bukti: POC N 35 PASS · 0 FAIL; `gate_residue --check` nol residu)

### KN-N-PERM-WIPE — POC FASE N MENGHAPUS baris matriks izin hasil seed
- Tanggal       : 2026-08-24
- Modul         : backend/test_core_notifikasi_alamat_poc.py (N6) · permission_settings
- Gejala        : POC FASE G-8 melapor `121 PASS · 1 FAIL` — "invarian GLOBAL tetap HIJAU"
                  gagal karena `verify_data_integrity` merah: *"kanonik
                  `permission_settings` KOSONG → seed GAP atau DRIFT"*. G-8 tidak bersalah;
                  ia hanya POC PERTAMA yang berjalan sesudah POC N.
- Akar masalah  : N6 mencabut `purchase_order.create` dari matriks lalu "memulihkan" dengan
                  `delete_one({"id": "default"})`. Benar untuk PERILAKU (kode jatuh ke
                  matriks bawaan) tetapi SALAH untuk DATA: baris hasil seed ikut lenyap.
                  Kelas cacat "pulih di mata pemeriksanya sendiri, rusak bagi pemeriksa lain".
- Perbaikan     : dokumen aslinya direkam LENGKAP sebelum disunting, lalu dipulihkan apa
                  adanya (dua tempat: akhir N6 dan blok `finally`).
- Invariant     : INV-GATE-01 (+ lapisan koleksi kanonik `verify_data_integrity`)
- Gate penangkap: `permission_settings` kini DIPANTAU `gate_residue.WATCH` — residunya
                  tak lagi bisa lolos tanpa terlihat (dulu koleksinya tidak dipantau
                  sama sekali, itulah sebabnya ia bertahan)
- Severity      : P1 (membuat POC lain memerah karena keadaan WARISAN, bukan kodenya)
- Status        : FIXED (bukti: G-8 `122 PASS · 0 FAIL`; `verify_data_integrity`
                  `PASS 241 · FAIL 0 · WARN 0`)

### KN-N-SORD-NOTIF — PO custom yang menunggu keputusan tanpa satu pun penerima
- Tanggal       : 2026-08-24
- Modul         : services/notification_service.py
- Gejala        : `audit_md_erp_readiness` BELUM: "notifikasi PO custom (special_orders):
                  0 notifikasi untuk 3 dokumen". Endpoint pembuatannya SUDAH memberi tahu
                  (FASE N), tetapi hanya untuk dokumen yang lahir SESUDAH fase itu — semua
                  dokumen yang sudah menunggu (termasuk seluruh data demo) tetap sunyi.
- Perbaikan     : job notifikasi menilai KEADAAN, bukan kejadian —
                  `_notify_pending_special_orders()` menyapu `status=pending_approval` dan
                  mengalamatkannya ke `required_approval_role` + pemegang `order.approve`
                  (fan-out per orang, `dedupe_scope="day"`).
- Invariant     : INV-NOTIF (alamat dari wewenang, nol siaran "all")
- Gate penangkap: POC FASE N butir **N3b** (dokumen warisan dapat penerima · penerima ⊆
                  pemegang wewenang · nol "all" · job dua kali tetap satu pesan per orang)
- Severity      : P2 (dokumen termahal bila terlambat, tetapi tidak merusak data)
- Status        : FIXED (bukti: 4 notifikasi untuk 3 dokumen; readiness `SELESAI=96 ·
                  BELUM=0 · DRIFT=0`)

### KN-CFG-SHELL — "Kembalikan ke global" meninggalkan cangkang setelan kosong
- Tanggal       : 2026-08-24
- Modul         : services/config_resolver.py (`clear_layer`)
- Gejala        : mencabut override per badan usaha menyisakan baris `system_settings`
                  ber-`scope=<entitas>` yang isinya `{"ar": {}}` — nol setelan. Tidak
                  mengubah perilaku (pembaca jatuh ke global) tetapi BERTAMBAH permanen
                  setiap kali override dicabut, dan layar Pusat Pengaturan menampilkan
                  "ada override untuk PT ini" yang isinya kosong.
- Akar masalah  : `$unset` pada jalur bersarang membuang NILAINYA, bukan WADAHNYA.
- Perbaikan     : sesudah `$unset`, wadah kosong ikut dibuang; bila tak ada satu pun
                  setelan bermakna yang tersisa, barisnya dihapus. Override LAIN pada
                  badan usaha yang sama tetap utuh (diuji dua kasus).
- Invariant     : INV-GATE-01 (`system_settings` sudah dipantau)
- Severity      : P2
- Status        : FIXED (bukti: kasus1 3→4→3 tanpa cangkang · kasus2 baris tetap ada
                  dengan override yang masih berlaku)

### KN-GATE-HOTRELOAD — mengukur gate sambil menyunting `backend/` = pengukuran tercemar
- Tanggal       : 2026-08-24
- Modul         : proses kerja (bukan kode produk)
- Gejala        : satu `gate.sh --full` MERAH dua tempat sekaligus — `system_settings
                  3 -> 7 (+4)` dan POC G-6b (yang menuntut `WARN 0`) tersandung
                  `INV-GL-DRIFT` — sementara tiga gate penuh lainnya HIJAU dan tiap POC
                  bersih saat dijalankan satu per satu.
- Akar masalah  : berkas di `backend/` disunting SELAGI gate berjalan → uvicorn hot-reload
                  → `bootstrap.run_bootstrap()` jalan DI TENGAH gate dan melahirkan
                  `system_settings` `makloon` · `receiving` · `hr` · `alerts/lock`
                  (+4, tepat) di antara dua pengukuran sidik jari.
- Perbaikan     : `gate_residue.py` kini MENYEBUT jebakan ini di pesan gagalnya, supaya
                  sesi berikutnya tidak membuang waktu memburu residu yang bukan residu.
- Severity      : P2 (mencemari bukti, bukan data)
- Status        : FIXED (bukti: gate penuh HIJAU tanpa penyuntingan selagi berjalan)
