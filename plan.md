...
---
## §STATUS D — SELESAI (2026-08-20): **FASE D DITUTUP** — 9 gate merah warisan ditutup di akarnya · peran ke-7 `designer` jadi sempit-tapi-jujur · POC FASE D lahir (55/55)

> Permintaan pemilik: *"saya ingin anda lanjutkan development dari repo ini, sebelumnya
> development terhenti disini."* Keputusan pemilik sesi ini: **(1)** tutup FASE D sampai
> gate **100% hijau** (bukan sebagian) · **(2)** wilayah `designer` tetap **SEMPIT** —
> panel yang bukan wewenangnya **DISEMBUNYIKAN**, dan rapor desainer diganti endpoint
> **"rapor SAYA sendiri"** · **(3)** dua gate merah non-FASE-D diperbaiki **sekarang**.

### 0. Titik henti diukur ulang lebih dulu — dan ternyata TIDAK bersih
Warisan sesi lalu: FASE D "terbangun" (koleksi `design_requests` 4 dok · service · router ·
4 layar · akun `designer@` · izin · nav hub) dan baris terakhir yang sedang dikerjakan
adalah `grep "### internal_requests" ENTITY_REGISTRY.md` — yaitu sedang mencari **template
entri registry**. Yang terukur hari ini:

| Klaim / kesan warisan | Hasil ukur empiris |
|---|---|
| FASE D "sudah jalan" | **SEBAGIAN** — `gate.sh --full` **MERAH 9 dari 120** |
| `audit_md_erp_readiness --fase D` | 9/10 SELESAI, 1 BELUM (`ENTITY_REGISTRY.md`) |
| POC FASE D (§D.D mewajibkan) | **TIDAK ADA** berkasnya |

### 1. Sembilan gate merah — dikelompokkan ke TIGA akar, bukan sembilan cerita
**Akar A — peran ke-7 lahir tanpa didaftarkan di tempat yang MENGHITUNG peran.**
* `scripts/check_nav_map.py:52` menyimpan `ROLES = [6 nama]` **hardcode**, sementara
  `parse_role_home()` membaca beranda peran dari SSOT (`navMeta`) yang SUDAH punya
  `designer` → `reach['designer']` **KeyError → gate CRASH**. Gejalanya bukan "merah
  dengan penjelasan" melainkan **traceback**: gate yang meledak tidak memeriksa apa pun
  sesudah baris itu (CHECK 4 & 5 tak pernah jalan).
  **Obat:** daftar peran **DIBACA** dari `frontend/src/config/roles.js ROLE_REGISTRY`, dan
  bila blok itu tak terbaca gate **MELEDAK BERISIK** (`sys.exit(2)`) — sengaja menolak
  daftar cadangan, karena cadangan = menghidupkan ulang sumber kedua.
* POC E-8 G1 mengunci **angka 6** di 6 asersi + `role_registry --self-test` `len(ROLES)==6`.
  **Obat:** yang dijaga E8.1 bukan *populasi* peran melainkan **paritas server↔layar**.
  Asersi diubah menjadi paritas + "peran inti lengkap"; daftar-putih nama peran di parser
  FE (sumber KEEMPAT) dihapus sehingga peran terbaru justru **ikut** diperiksa.
  Bukti: POC **64/64** · self-test **"HIJAU — 7 peran, 0 gagal"**.
* `audit_sales_roles_ux` menemukan **3 PANEL MATI** milik `designer` — bug UX NYATA
  (dropdown 403 ditelan `.catch`, persis kelas yang komentar `permissions_config.py`
  sudah tuliskan untuk finance). **Dua di antaranya diperbaiki DI AKARNYA, bukan
  dengan pembebasan:** `/suppliers` & `/warehouses` **menumpang** di modul bersama
  `features/rnd/rndApi.js` yang diimpor layar milik designer padahal **tak pernah
  dipanggil** — `listWarehouses` bahkan **NOL PEMAKAI** (mati) dan `listSuppliers` hanya
  dipakai `SampleSendModal.jsx`. Pembebasan di `TERGATED_DI_KODE` akan membuat gate hijau
  **atas alasan yang tidak benar** ("dipagari izin"), padahal sebabnya "endpoint salah
  tempat". Endpoint dipindah ke dekat pemakainya → tuduhan hilang tanpa satu baris
  pembebasan. Panel ke-3 → lihat §2.

**Akar B — pintu keputusan `design_request` tak terdaftar di penjaga.**
`INV-APPR-01` (pintu `/approve`+`/reject` tak ada di `DOOR_QUEUE`) dan `INV-HOME-01`
(baris antrean tanpa pasangan hitung-ulang di `verify_home_kpi.EXPECT`) merah, lalu
**merembes** ke POC F-6 (4 gagal) & F-6.7 (2 gagal) yang memang mengasersikan "gate itu
HIJAU". Satu kegagalan F-6 ("total tidak ikut bertambah karena harga langganan")
**diperiksa sendiri, bukan diasumsikan rembesan** — dan memang hilang begitu selisih
hitung (api=29 vs mandiri=28) tertutup. Ketiga tabel hitung-ulang (backend `QUEUES`,
`verify_home_kpi.EXPECT`, POC F-6) sengaja **ditulis ulang masing-masing** = tiga opini
independen; `revision` **TIDAK** dihitung karena bolanya sudah kembali ke desainer.
Bukti: 221 & 80 cek hijau · F-6 **43/43** · F-6.7 **41/41**.

**Akar C — dua gate merah yang TERBUKTI BUKAN BUG KODE (dan pengakuannya penting).**
POC E-4 `"Kategori Biaya: 0 baris global terlihat dari KSC"` dan POC E-8 G2/G3
`"E8.3 sales lihat kunjungannya sendiri → HTTP 404"` **hijau tanpa satu baris kode
aplikasi diubah** (E-4 **56/0**, E-8 desk **97/97**). Sebabnya **prosedur pemulihan saya
sendiri**: koleksi fondasi (`expense_categories`, `gl_accounts`, `hr_employees`) HANYA
lahir dari `bootstrap.run_bootstrap()` **saat backend START** — bukan dari
`seed_realistic.py`. Kontainer datang dengan **mongodb MATI**, jadi bootstrap menulis ke
ketiadaan; lalu DB di-drop tanpa restart. Gate benar; **lingkungannya separuh jadi**.
Kelas jebakan ini sudah tercatat di handoff (K1: "`uoms` 4 baris, CM/INCH hilang tergantung
urutan restart vs seed") tetapi **belum pernah dijaga skripnya**. → `.restore_env.sh`
ditulis ulang: mongodb **dipastikan HIDUP** sebelum backend di-restart, plus langkah
**VERIFIKASI FONDASI** yang **gagal-berisik** + saran perbaikan satu perintah.

### 2. Panel mati ke-3 disembuhkan dengan MEMBERI, bukan menyembunyikan
Rapor lintas desainer menilai ORANG → wewenang atasan, jadi `designer` hanya bisa 403 =
panel mati. Tetapi *"tidak boleh melihat rekan"* **tidak sama dengan** *"tidak boleh
melihat dirinya"*. Lahir **`GET /api/design/reports/mine`** — dipagari
`design_request.**view**` (bukan `.report`), karena melihat angka diri sendiri bukan
wewenang penilaian (pola yang sama dengan `/rnd/reports/my-kpi` & `/hr/attendance/me`).
Server mengirim **baris miliknya + rata-rata tim agregat**, `items` **absen**, nol nama
rekan (privasi **PS-18**). Layar: tab **"Rapor Saya"** (`MyDesignerReportPanel.jsx`) —
komponen TERSENDIRI, bukan `prop` pada `DesignerReportPanel`, supaya tidak ada satu
komponen memegang dua izin (di situlah kebocoran nama rekan akan lahir).
Satu jebakan yang sengaja dihindari: `report_mine()` **menolak** query yang sudah disaring
`assigned_to` — kalau disaring, `total_designers` selalu 1 dan `rank` selalu 1: angka yang
**terlihat masuk akal tetapi tidak berarti apa-apa**.

### 3. POC FASE D lahir (§D.D) — `backend/test_core_design_request_poc.py`
**55 PASS / 0 FAIL, 4× berturut, NOL RESIDU.** Membuktikan D1 alur ujung-ke-ujung ·
D2 alasan revisi wajib **di server** · D3 serah hasil harus menunjuk artwork NYATA di
badan usaha yang sama · D4 rapor **== hitung-ulang MANDIRI** dari MongoDB · D5 masuk
antrean keputusan + **menjalankan** `check_nav_map`/`INV-APPR-01`/`INV-HOME-01`
sungguhan · D6 `refs` **dua arah** · D7 mode gabungan **409** · D8 peran ke-7 sempit tapi
jujur (IDOR 403 · tak boleh assign/approve/reject/create · rapor rekan 403 · **rapor SAYA
200 tanpa nama rekan**) · D9 gate **MEMBACA** peran ke-7 dari registry (hijau karena
memeriksa, bukan karena buta) · D10 **nol residu DIUKUR**.
Tiga asumsi saya sendiri yang **terbukti salah saat POC dijalankan** (dan itu gunanya POC):
`approvals.rows` → sebenarnya **`approvals.items`** · relasi di koleksi `doc_relations` →
sebenarnya **di dalam `refs[]` dokumen** (dua arah `parent`/`child`) · pembersihan audit
per-field → harus **pola SELISIH id** karena `POST /auth/login` sendiri menulis 1 audit +
1 sesi per akun (kelas residu yang sama sudah pernah menggigit POC E-8 & U).

### 4. Bukti penutup FASE D
`gate.sh --full` **SEMUA GATE HIJAU 2× berturut** (309s & 311s · **145 PASS / 0 FAIL**) ·
POC FASE D **55/55** (4×, nol residu) · POC E-8 G1 **64/64** · E-8 desk **97/97** ·
E-4 **56/0** · F-6 **43/43** · F-6.7 **41/41** · `INV-APPR-01` **221/0** ·
`INV-HOME-01` **80/0** · `audit_sales_roles_ux` **HIJAU (7 peran, nol layar/panel mati)** ·
`check_nav_map` **PASS** (dulu CRASH) · `verify_data_integrity` **237/0/0** ·
`validate_compliance` **22 PASS / 0 FAIL** · `audit_md_erp_readiness --fase D`
**SEMUA SELESAI** · build FE bersih.
**Uji lewat layar (dijalankan SENDIRI di peramban, bukan hanya oleh agen uji):**

| User story | Bukti di layar |
|---|---|
| **D-US1** MD buat permintaan dari SO → tugaskan → tenggat | `KSC/DSR-00014` dibuat lewat modal (TOTAL 4→5), lalu ditugaskan ke **Sari Melati** tenggat **2026-08-27**; `dsr-detail-status` = **Ditugaskan**; riwayat memuat *"Ditugaskan ke Sari Melati … — tenggat 2026-08-27"* |
| **D-US2** desainer unggah/serahkan artwork → masuk antrean atasan | Sari Melati memilih **DSG-PARANG-02** → status **Menunggu Keputusan**, toast *"Hasil diserahkan — menunggu keputusan atasan"*; beranda manajer menampilkan **"Permintaan desain menunggu keputusan 1"** |
| **D-US3** manajer minta revisi ber-alasan; desainer membacanya | Pop-up *"Apa yang harus diubah pada KSC/DSR-00003? Alasan ini dibaca desainer."*; tombol kirim **DISABLED** selama alasan kosong; sesudah kirim → status **Minta Revisi** + baris **"Alasan revisi terakhir"**; **desainer membaca alasan itu di dokumennya** |
| **D-US4** rapor | Manajer: **Rapor Desainer** (Sari Melati 4·1·2·1·2·0·0,02·⭐4,5·25% + baris "Belum ditugaskan"). Desainer: **Rapor Saya** (3·2·1·1·0 · ⭐4,5 · 33,3% · "1 dari 1" · *"1 desainer aktif · rata-rata …"*) **tanpa satu nama rekan** |
Regresi peran sempit terlihat di layar: sidebar desainer hanya **Desainer + Profil Saya**;
papannya **3 dokumen** vs manajer **4**; nol tombol ACC/revisi/tugaskan/buat.
**Data demo dipulihkan** ke 4 dokumen (`seed_realistic.py` → `seed_e9_chain_demo.py`),
integritas tetap **237/0/0**.

### 5. Opini kedua agen uji (iterasi 236 backend · 237 frontend)
Backend **18/20, nol bug kritis**. Frontend **7/9 PASS**, dan yang paling berharga:
**FE-7 "zero dead panels — NOL respons 403"** pada ketiga layar desainer, serta **FE-9
regresi bersih** (daftar supplier di modal Kirim Sample **masih terisi** sesudah
`listSuppliers` dipindah). Satu temuan `LOW` ("KNSelect penugasan tidak update") saya
**REPRODUKSI dan TERBUKTI TIDAK ADA**: memakai testid kanonik
`dsr-assign-select-option-user_designer_01` → nilai berubah *"Sari Melati"*, tombol aktif,
status **Ditugaskan**. Sebab tuduhannya: agen memilih baris **`-option-empty`**
("Pilih desainer") yang memang mengosongkan pilihan. Agen uji **meninggalkan
`KSC/DSR-00005`** (dua kali) — dibersihkan tangan, seperti biasa.

### 6. Jebakan untuk sesi berikutnya (BACA)
* **`KNSelect` merender placeholder SEBAGAI OPSI** (`{testId}-option-empty`, mis.
  "Pilih desainer"). Memilihnya = mengosongkan pilihan → tombol simpan mati. Selalu pakai
  testid kanonik **`{testId}-option-{value}`**, jangan `[role="option"]` generik. Ini sudah
  memproduksi satu laporan bug palsu.
* **Ada DUA "POC FASE D"** di `gate.sh`: `POC FASE D · PERMINTAAN DESAIN (DSR)` (fase ini)
  dan `POC FASE D (makloon rantai proses)` (penomoran lama, `test_fase_d_makloon_poc.py`).
  Label sudah dibedakan — jangan disatukan.
* **Koleksi fondasi hanya lahir saat backend START.** Kalau POC tiba-tiba merah dengan
  gejala "0 baris global" / "404 profil karyawan", **jangan cari bug di kode**: jalankan
  `supervisorctl restart backend` (bootstrap idempotent) lalu ukur ulang.
  `.restore_env.sh` sekarang memeriksanya sendiri.
* **Menambah peran ke-8?** Cukup `backend/role_registry.py` + `frontend/src/config/roles.js`
  (+ `ROLE_NAV` bila menunya beda). Gate & POC sudah membacanya dari registry — jangan
  menambah daftar peran hardcode baru di mana pun.
* **Endpoint baru jangan ditaruh di modul API bersama** kalau hanya satu layar
  memakainya: `audit_sales_roles_ux` menilai layar dari **penutupan impor**-nya, jadi
  endpoint yang menumpang akan membuat layar lain dituduh "panel mati" — dan pembebasan
  adalah obat yang salah untuk sebab itu.
* **Sisa DRIFT (bukan bagian FASE D):** D2 `qc_inspections` hantu (FASE I) · D3 PO tak
  menaut PR (**P-0**, prasyarat FASE P) · D5 11 notifikasi `recipient_role="all"` (FASE N) ·
  D6 `REL_LABEL` memuat `applied_to` yang tak ada di `REL_INVERSE` (FASE I) ·
  D7 nomor dokumen kembar di `md_samples` (FASE S).

### 7. Berikutnya
**P-0 — tautan PO→PR** (keputusan pemilik: **hanya dokumen BARU**, tanpa backfill), lalu
**FASE S (Sampling)** → **I (Inspeksi/QC)** → **P (Papan PO)** → **N (Notifikasi)** →
**M (Makloon)**. P-0 adalah **prasyarat FASE P**: tanpa tautannya, "Nama Sales" di papan
PO mustahil dirunut (terukur: **0/14** PO menyimpan `pr_id`).

---
## §STATUS P-0 + P (diwarisi, DIVERIFIKASI 2026-08-21): **tautan PO→PR & Papan PO per Lini sudah ADA**

Sesi ini mewarisi pekerjaan yang **sudah selesai tetapi belum pernah ditulis statusnya**
(sesi sebelumnya berhenti mendadak — lihat §STATUS REF-04 di bawah). Diukur ulang dengan
alatnya sendiri, bukan dipercaya dari prosa:

| Yang diwarisi | Bukti terukur hari ini |
|---|---|
| **P-0** tautan `PO → PR → SO` + kolom **Nama Sales** dirunut | `backend/test_core_p0_poc.py` **36/36** · penjaga `INV-ORIG-01` (`scripts/guardrails/verify_doc_origin.py`, self-test **26 kasus**) HIJAU · 3 dari 17 PO menyimpan `pr_id` (PO rutin **sengaja** kosong — keputusan pemilik "tanpa backfill") |
| **FASE P** Papan PO per Lini | `backend/test_core_po_board_poc.py` HIJAU · penjaga `INV-STAGE-01` (`verify_po_board.py`, self-test **22 kasus**) HIJAU · layar `frontend/src/features/purchasing/PoBoardView.jsx` + `PoStageModal.jsx` · akun uji berpagar lini `manager.printing@kainnusantara.id` |

**Catatan jujur:** keduanya TIDAK dikerjakan di sesi ini — hanya diverifikasi. Yang dikerjakan
sesi ini adalah titik henti yang membuat keduanya MEMERAH (di bawah).

---
## §STATUS REF-04 — SELESAI (2026-08-21): **"menghapus dokumen wajib menyapu tautan baliknya"** · kelas residu POC ditutup sampai akar

### 0. Titik henti yang diwarisi (satu baris merah)
Sesi sebelumnya berhenti tepat di sini:

```
python scripts/audit_doc_refs.py --strict
  ✗ [FAIL] Tautan menggantung: 2 temuan
```

Yang menyesatkan: POC E-7 **58/58 HIJAU**, `verify_data_integrity` **237/0/0 HIJAU**.
Jadi semua alat berkata "sehat" sementara gate berkata "merah" — dan tidak ada satu pun
pesan yang menghubungkan keduanya.

### 1. Akar masalah (bukan dugaan — dua ref-nya disebut namanya)
```
internal_request KSC/PIN-00003 --fulfilled_by--> interco_transaction KSC/IC-00007   (target tak ada)
internal_request KSC/PIN-00003 --fulfilled_by--> interco_transaction KANDA/IC-00007 (target tak ada)
```
Rantai sebabnya, seluruhnya:
1. `doc_refs_service.link()` menulis relasi **DUA ARAH** — keputusan yang benar.
2. Konsekuensinya lahir kewajiban yang **tak pernah ditulis di mana pun**: yang MENGHAPUS
   dokumen wajib menyapu ref yang menunjuk dokumen itu.
3. POC E-7d membuat `KSC/PIN-00003`, lalu "membatalkannya" lewat `POST /cancel` —
   yang **selalu gagal** begitu statusnya `converted` (pagar `_assert_open` yang memang
   benar). Kegagalan itu **ditelan diam-diam**, jadi POC mengaku bersih.
4. Pembersih POC menghapus dua draf antar-PT; permintaan yang tertinggal tetap memegang
   dua ref ke dokumen yang sudah tidak ada → panel **"Referensi Dokumen"** dengan tenang
   menawarkan dua tautan menuju **404**.
5. `gate_residue.py` tidak memantau `internal_requests`, jadi INV-GATE-01 pun ikut buta.
6. Di `gate.sh`, `audit_doc_refs` berjalan **SEBELUM** semua POC → hantu baru terlihat pada
   gate **BERIKUTNYA**, saat tak ada lagi yang menghubungkannya dengan penyebabnya. Itu juga
   yang membuat **POC P-0 memerah karena kesalahan POC LAIN**.

### 2. Yang dikerjakan
**(a) Pasangan `link()` yang hilang — di PRODUKSI, bukan cuma di POC.**
`doc_refs_service` mendapat `unlink_all()` · `safe_unlink_all()` · `dangling_refs()` (satu
definisi, dipakai bersama audit·gate·penjaga). Audit kode menemukan **5 titik penghapusan
PRODUKSI** dengan lubang yang sama — semuanya ditutup:

| Titik | Akibat kalau dibiarkan |
|---|---|
| `po_amendment_service` menghapus inbound `wms_tasks` (doc_type `grn`) | **PO yang MASIH HIDUP** memegang ref ke penerimaan yang tak ada — tiap amandemen PO menambah tautan 404 |
| `purchase_return_service.delete_purchase_return` | PO memegang `reversed_by` ke retur yang sudah dihapus |
| `routers/supplier_contracts.delete_contract` | order makloon menaut kontrak lewat `contract_id` tingkat atas, sedangkan pagar hapus hanya memeriksa `steps.contract_id` |
| `routers/sales_orders` rollback transfer (2 jalur `except`) | disatukan jadi `_rollback_created_transfers()` supaya sapuannya tak mungkin lupa disalin ke salah satu jalur |

**(b) Penjaga baru `INV-REF-04`** — `scripts/guardrails/verify_ref_unlink.py`: statik lewat
`ast` (bukan regex, jadi komentar/string tak bisa mengelabui), resolusi konstanta
**lintas-modul** (`db[cs.COLL]`), koleksi dinamis wajib terdaftar beserta **alasan tertulis**
(bukan pembebasan buta), `--self-test` **8 kasus dua arah** termasuk anti-tuduh-palsu.

**(c) Kelas residu POC ditutup dengan MENGUKUR, bukan menebak.** Alat baru
`scripts/ukur_residu_poc.py` menjalankan tiap POC gate satu per satu dan menghitung selisih
**SELURUH koleksi**. Hasil ukur pertama: **14 dari 33 POC meninggalkan residu** — semuanya di
koleksi yang tak dipantau siapa pun:

| POC | Residu terukur (per satu kali jalan) | Obat |
|---|---|---|
| E-7 antar-PT | kas **+7** · jurnal **+9** · pinjaman **+2** · aset tetap **+2** · eliminasi **+1** · PR **+2** · permintaan internal **+1** · **2 tautan hantu** | `poc_stock_guard.MONEY_COLLECTIONS` (pulih EKSAK) + hapus dokumennya sendiri + **pemeriksaan nol residu SELURUH basis data** |
| FASE T tahapan | jurnal **+2** · **3 tautan hantu** (`KSC/SCT-00004`, `KSC/SCT-00008` → SPK yang dihapus) | `FULL_COLLECTIONS` + `sweep_ghost_refs()` |
| G-2 denda · G-3 selisih | jurnal **+1** masing-masing | `FULL_COLLECTIONS` |
| G-9 pusat kasus | jurnal **+3** · kas **+1** · **7 tautan hantu** (`SO-0008`, `KSC/SO-00010` → kasus & kwitansi yang dihapus) | snapshot jurnal+kas + `sweep_ghost_refs()` |
| G-6b faktur pajak internal | **4 tautan hantu** (`IC-00002` → faktur pajak, `IC-00006` → retur antar-PT) | `sweep_ghost_refs()` |
| FASE D makloon | `system_settings` **+1**; dan filter notifikasinya memakai `type` padahal fieldnya `notif_type` — jadi POC **tak pernah** membersihkan notifikasinya, sementara membetulkan nama field saja akan **menghapus notifikasi DATA DEMO** | sapuan dibatasi ke SPK milik POC (`action_id`/`ref`) + snapshot `system_settings` |
| G-0 konfigurasi · F-1 penerimaan · G-7 kontrabon | `system_settings` **+2/+1/+1** (dokumen scope baru: `receiving`, `ent_ksc`) | snapshot `CONFIG_COLLECTIONS` |
| rantai retur | `store_credit_ledger` **+1** (**SALDO UANG pelanggan**) | snapshot |
| FASE F R&D · F US3 | `approval_matrix_log` **+2** · `credit_overrides` **+1** | snapshot |

**(d) Kebutaan gate ditutup permanen.** `gate_residue.py`: daftar pantauan **+11 koleksi**
dan pemeriksaan **tautan menggantung** yang **TIDAK pernah** diabaikan `--ignore-trails`
(hantu bukan "jejak append-only", ia dokumen hidup yang menunjuk ketiadaan). Karena
`--check` berjalan SESUDAH POC, kebutaan urutan itu ikut hilang.

### 3. Bukti penutup
* `gate.sh --full` **106 gate HIJAU / 0 FAIL, DUA KALI BERTURUT** (325 s & 327 s)
* POC E-7 **62/62** (dari 58/58 — dua pemeriksaan residu baru + satu pemulihan buku uang),
  dijalankan **2× berturut** dengan `internal_requests`/`interco_transactions`/`audit_logs`/
  `notifications` kembali **persis** ke angka awal
* `audit_doc_refs --strict` **32 PASS / 0 FAIL** · `verify_data_integrity` **237/0/0**
* `INV-REF-04` self-test **8/8**; pada kode SEBELUM perbaikan penjaga ini menghasilkan
  **5 pelanggaran produksi** (bukti-merah, bukan klaim)
* `ukur_residu_poc.py`: **nol** residu non-jejak di **33 POC** (sisa hanya `audit_logs`/
  `notifications` = jejak append-only, sesuai keputusan lama yang memang disengaja)

### 4. Jebakan untuk sesi berikutnya (BACA)
* **POC baru yang MENGHAPUS dokumen bernomor WAJIB** memanggil
  `poc_stock_guard.sweep_ghost_refs()` di cleanup-nya. Kalau lupa: `gate_residue --check`
  akan menyebut nomor dokumen & hantunya — tetapi baru **sesudah** blok POC, jadi jangan
  panik mencari bug di kode fitur.
* **POC baru yang menjalankan alur BERJURNAL wajib** `snapshot_stock(FULL_COLLECTIONS)`
  (stok **dan** jurnal **dan** kas). Memulihkan satu sisi saja melahirkan residu jenis baru —
  itu pelajaran POC-RESIDU-02 yang terulang lagi tahun ini.
* **Jangan pernah** memulihkan `number_sequences`: penghitung nomor dokumen HARUS monoton,
  memulihkannya = memakai ulang nomor surat (kelas INV-NUM-01, nomor kembar).
* **`asyncio.run()` hanya SEKALI per proses POC.** Klien motor di `db.py` mengikat diri ke
  event loop pertama; pemanggilan kedua mati `RuntimeError: Event loop is closed` — dan
  kalau itu terjadi DI DALAM blok pembersihan, POC justru **menambah** residu (terukur
  +17 `audit_logs`) sambil tetap melaporkan "0 FAIL".
* **Ukur, jangan menebak koleksi mana yang bocor**: `python scripts/ukur_residu_poc.py`
  (bisa `--only <kata>`). Menebak dari nama koleksi sudah dua kali salah.

### 5. Berikutnya
Roadmap tak berubah: **FASE S (Sampling)** → **I (Inspeksi/QC)** → **N (Notifikasi)** →
**M (Makloon)**. Utang DRIFT yang masih tercatat: D2 `qc_inspections` hantu (FASE I) ·
D5 11 notifikasi `recipient_role="all"` (FASE N) · D6 `REL_LABEL` memuat `applied_to`
yang tak ada di `REL_INVERSE` (FASE I) · D7 nomor dokumen kembar di `md_samples` (FASE S).

---
## §STATUS S — SELESAI (2026-08-23): **satu permintaan, BEBERAPA JENIS sampling** · layar ikut plural · dua pembaca LAMA yang membuat gate merah ditutup

### 0. Titik henti yang diwarisi — DIUKUR, bukan ditebak
Sesi sebelumnya berhenti **di tengah pekerjaan frontend FASE S**: editan terakhirnya
menambahkan `import { roundTypeOf } from "./sampleTypeMeta"` di `RndReportsView.jsx`.
Yang saya ukur lebih dulu (sebelum menyentuh satu baris pun):

| Diukur | Angka |
|---|---|
| `.restore_env.sh` (deps · mongo · bootstrap · seed · build FE) | `BUILD_EXIT=0`, semua servis RUNNING |
| POC FASE S `backend/test_core_sampling_poc.py` | **65 PASS / 0 FAIL** → backend FASE S memang sudah selesai |
| `.emergent/emergent_todos.json` | 8 todo `completed` (S.A · S.B · S.B2 · migrasi · D7 · gate · POC), **1 `in_progress` + 5 `pending` — semuanya frontend & bukti** |
| `gate.sh --quick` | HIJAU (20s) |
| **`gate.sh --full`** | **MERAH** — dan inilah yang tidak pernah tercatat: 13 gate gagal |

### 1. Frontend: pembaca plural yang belum sempat dikerjakan
| Berkas | Sebelum | Sesudah |
|---|---|---|
| `rnd/RndReportsView.jsx` | label papan SLA dari peta hardcode `SAMPLE_TYPE_LABEL[r.type]` | `typeLabel(r.type, types)` — `types` dari `/api/rnd/sample-types` |
| `rnd/RndSpecsView.jsx` | kolom "Jenis Sample" dari peta hardcode | `typeLabel(...)` + `types` dari `/api/rnd/meta`, diteruskan ke form & panel (satu kosakata untuk tiga permukaan) |
| `rnd/SpecFormModal.jsx` | `TYPE_OPTS` **hardcode** 3 baris (`labdip`·`proofing`·`bulk_sample` — yang terakhir SUDAH dinonaktifkan pemilik) + "wajib desain" ditebak `if hint == "proofing"` | pilihan dari MASTER (`typeOptions`), kewajiban desain dari `typeMeta().requires_design`, dan jenis bawaan digeser otomatis bila jenisnya tidak sah di badan usaha itu |
| `rnd/SpecDetailPanel.jsx` | "Rencana sample" & daftar sample turunan dari peta hardcode | `typeLabel(...)` (prop `types`) |
| `rnd/RndSamplesView.jsx` | deep-link dari Galeri Desain memaksa `sample_types: ["proofing"]` | hanya mengirim PERTANYAANNYA (`need_design`); form menjawabnya dari master |
| `designer/DesignerSlaPanel.jsx` | peta hardcode di layar | **label dari SERVER** (`sample_type_label`) |
| `rnd/sampleTypeMeta.js` | — | `typeOptions()` + `FALLBACK_TYPE_CODES` (cadangan hanya jenis baku yang masih AKTIF — `bulk_sample` sengaja tidak ditawarkan) |

**Kenapa label papan eskalasi SLA dipindah ke server, bukan dibaca layar seperti yang lain:**
layar itu milik menu Desainer, dan peran `designer` **tidak punya izin `rnd.view`** —
kalau panelnya memanggil `/api/rnd/sample-types` sendiri, panelnya mati 403 (kelas
"panel mati" yang sudah dua kali menggigit di FASE D). Jadi `rnd_sla_service` yang
membaca master (`mreg.sample_types()` — termasuk jenis yang sudah **dinonaktifkan**,
supaya dokumen lama tetap punya nama manusia) dan mengirim `sample_type_label`.
**Catatan jujur:** field `sample_type` pada baris papan itu BUKAN field dokumen yang
dihapus migrasi — ia dihitung server sebagai jenis ROUND, jadi papan lama tidak pernah
kosong; yang salah hanya kosakatanya untuk jenis yang DITAMBAH pemilik.

### 2. Akar `gate.sh --full` MERAH: dua pembaca LAMA yang belum ikut pindah
Bukan bug baru dari pekerjaan frontend — dua alat yang masih memakai kosakata
pra-FASE-S, plus satu **rantai akibat** yang membuat 9 POC lain ikut memerah:

**(a) `INV-RND-02` menuduh data yang SEHAT.** Invarian itu menuntut `round_no` berurut
**per supplier**. Sejak FASE S nomor round berjalan per **(supplier × jenis)** —
`rnd_sample_service.open_round()` menomori begitu dan kuota `rnd.max_rounds` pun
dihitung per jenis. Buktinya di data demo: `KSC/SMP-00001/sup_d1e8531b45be: nomor round
tidak berurut [1, 1]` — dua round jenis BERBEDA pada supplier yang sama. Diperbaiki
menjadi per (supplier × jenis), dengan pengikatan round lama tanpa `type_code` ke jenis
pertama dokumennya — **satu definisi**, cermin `_type_of_round()`.

**(b) POC FASE F (`test_fase_f_rnd_poc.py`) masih hidup di dunia lama.** Dua hal:
* ia menuntut `bulk_sample` ADA di enum jenis, padahal pemilik menonaktifkannya
  (keputusan #4) dan `handfeel` yang baru justru tidak pernah diperiksa;
* ia mengirim hasil ukur pilihannya sendiri (`delta_e` + `gsm_actual` + `shrinkage_pct`)
  untuk round **labdip**. Sejak FASE S kolom hasil ukur lahir dari master dan
  divalidasi **dua arah** (field yang tidak diminta DITOLAK, yang diminta WAJIB ada) —
  jadi servernya menjawab **400** pada POC, bukan pada kode produksi. POC diubah
  membaca master (`meas_for()` mengisi seluruh `measurement_fields` dari kamus
  `/api/rnd/meta.measurements`, lalu menimpa nilai yang memang sedang diuji).

**(c) Rantai akibatnya — pelajaran mahal yang layak dicatat.** POC F **crash**
(`KeyError: 'rounds'`) tepat setelah 400 itu, jadi blok pembersihannya **tidak pernah
jalan**. Residunya (3 `sales_orders` data demo terhapus + `approval_matrix_log` +1)
membuat **9 POC berikutnya di gate yang SAMA** ikut memerah (G-2 · G-3 · G-4 · G-6b ·
G-7 · G-8 · G-9 · P · F-US3) beserta `INV-GATE-01` dan `verify_data_integrity`.
Nol dari 9 kegagalan itu punya sebab sendiri. Obatnya: perbaiki akar → `seed_realistic.py`
→ ukur ulang.

### 3. Bukti penutup (angka, bukan klaim)
* `gate.sh --full` **SEMUA GATE HIJAU, DUA KALI BERTURUT** — 339s & 340s (110 baris receipt)
* POC FASE S **65/65** (2× berturut, nol residu) · POC FASE F **100/0** (dari crash)
* `verify_data_integrity` **237 PASS / 0 FAIL / 0 WARN**; `INV-RND-02` kini berbunyi
  *"52 dari 56 round tertutup … nomor round berurut per (supplier × jenis sampling)"*
* `audit_md_erp_readiness --fase S`: **seluruh baris S SELESAI** (nol BELUM/DRIFT milik S)
* `validate_compliance` **22 PASS / 0 FAIL** (21 WARN lama: ukuran berkas & impor tak
  terpakai di berkas yang tidak saya sentuh)
* `eslint` **0 error** · `esbuild` bundle bersih · `rebuild_frontend.sh` **BUILD OK**
* Bukti kecil master→layar (`.logs/_verify_fase_s_ui_master.py`): jenis baru
  `wash_test_probe` yang ditambah lewat API **langsung** muncul di
  `/api/rnd/sample-types` **dan** `/api/rnd/meta` dengan NAMA-nya (bukan kode teknis),
  `measurement_fields`-nya terbawa, lalu **dihapus lagi → nol residu**: **7/0**
* **Agen uji UI (iterasi 238)**: **S.F-1 LULUS penuh** — chip multi-jenis (bukan dropdown),
  dua jenis terlihat di baris daftar, penyaring jenis dari master berfungsi, `bulk_sample`
  **tidak** ditawarkan, label berupa nama manusia. Nol bug UI/integrasi/desain dilaporkan.
  Sisa user story tidak dijalankannya (ia menebak `data-testid` yang tidak ada, mis.
  `purchase-orders-view`) — **saya jalankan sendiri di peramban**, hasilnya di bawah.
  Dokumen uji yang ditinggalkannya (`KSC/SMP-00029` "TEST MULTI-TYPE") **sudah dihapus** →
  `md_samples` kembali **28** dokumen.
* **Uji layar SENDIRI (peramban):**

| User story | Bukti di layar |
|---|---|
| **S.F-1** dua jenis paralel | `KSC/SMP-00001` — kepala dokumen: *Labdip (Kain Polos) · Handfeel (Rasa & Konstruksi)*; panel **"Perbandingan supplier PER JENIS"** memuat DUA tabel; blok round terpisah `sample-type-block-{supplier}-{jenis}` **4 blok** (2 supplier × 2 jenis) dengan riwayat masing-masing |
| **S.F-2** tautan pesanan | kartu **"Untuk pesanan: tertaut — lihat Jejak dokumen"**; Jejak Dokumen menampilkan rantai `KSC/DSR-00003 → KSC/SPEC-00001 → KSC/SMP-00001 → KSC/SCT-00010 → SO-0005` dan relasi *"KSC/SMP-00001 **berasal dari** SO-0005 — permintaan pelanggan"* |
| **S.F-3** tutup round wajib bukti + ukur DARI MASTER | modal *"Setor hasil rnd 1 · **Labdip (kain polos)** · NTT Weaving Co"* berkata **"Kolom hasil ukur di bawah datang dari master Jenis Sampling untuk jenis Labdip (kain polos)"** dan menampilkan **ΔE · Tahan cuci (skala 1–5) · Tahan gosok (skala 1–5)** — bukan `gsm_actual` seperti sebelum FASE S; pita merah *"Belum ada bukti terunggah … server akan menolaknya"* + *"Masih kosong: ΔE, Tahan cuci, Tahan gosok"* |
| **S.F-4** jadi → kirim | kartu **"Sample jadi 2026-08-23"** dan **"Dikirim 2026-08-23 → Pelanggan (Tekstil Medan Jaya)"** pada `KSC/SMP-00001`; kartu ringkasan daftar menghitung **"Sudah jadi 1 · Menunggu dikirim 0"** dari server |
| label master di 4 permukaan | Laporan R&D papan SLA (*Labdip (kain polos)* / *Proofing (sample printing)*) · kolom Jenis Sample di Spesifikasi · form Spesifikasi Baru (**3 jenis dari master**, termasuk **Handfeel** yang dulu tak ada di dropdown, + peringatan *"Jenis Proofing (sample printing) menuntut kode desain"*) · **papan Eskalasi SLA menu Desainer** (3 baris, semuanya menyebut *Labdip (kain polos)*) |
| regresi 3 layar lama | **Pesanan** (8 pesanan, pencarian ada — diuji agen) · **Pesanan Pembelian** (12 PO, chip lini, unduh CSV, nol pesan galat) · **Status Stok & ATP** (20 produk, nol `undefined`/`NaN`) |

### 4. Jebakan untuk sesi berikutnya (BACA)
* **POC yang CRASH tidak membersihkan dirinya.** Satu `KeyError` di POC pertama bisa
  memerahkan 9 POC sesudahnya di gate yang sama. Kalau banyak POC merah sekaligus,
  **baca yang PERTAMA merah** dan jangan mendiagnosis yang lain sebelum residunya
  dibersihkan (`seed_realistic.py`) — 9 dari 13 kegagalan gate hari ini adalah gema.
* **Alat lama ikut menua.** Setiap kali satu field berubah dari tunggal → daftar
  (`sample_type` → `sample_types[]`), yang wajib ikut diperiksa bukan hanya
  layanan/layar, tetapi juga **invarian** (`verify_data_integrity`) dan **POC fase
  lama** yang menuliskan bentuk data lama. Keduanya terlihat hanya di `--full`.
* **`_validate_measurements` menolak field yang tidak diminta master.** Setiap
  skrip/seed/POC yang menuliskan sendiri daftar hasil ukur akan memerah begitu pemilik
  mengubah master. Bacalah `measurement_fields` — jangan menyalinnya.
* **Panel di menu peran lain jangan mengimpor `rndApi`.** `audit_sales_roles_ux` menilai
  layar dari penutupan impornya, dan peran `designer` tidak punya `rnd.view`. Kalau
  butuh kosakata master di layar peran lain: **kirim labelnya dari server**.
* **`bulk_sample` tidak boleh muncul di pemilih mana pun** (dinonaktifkan pemilik) tetapi
  **wajib tetap punya label** untuk dokumen lama — dua kebutuhan yang berbeda, dan
  `typeOptions()` vs `typeLabel()` memisahkannya.

### 5. Sisa yang BELUM (jujur, bukan disembunyikan)
* `md_samples.so_id` terisi **1 dari 28** dokumen demo. Formnya sudah punya isiannya
  (user story S.F-2 lulus di POC & layar), tetapi data demo lama memang tidak tertaut —
  ini kekosongan DATA, bukan kekosongan kode. Backfill sengaja tidak dilakukan
  (keputusan pemilik "tanpa backfill" masih berlaku).
* `frontend/src/features/rnd/rndMeta.js:SAMPLE_TYPE_LABEL` dipertahankan **hanya** sebagai
  cadangan sebelum master termuat & untuk dokumen lama; tidak ada layar yang membacanya
  langsung lagi (diperiksa: 0 pemakaian di luar `sampleTypeMeta.js`).

### 6. Berikutnya
**FASE I — INSPEKSI & QC SEBAGAI DOKUMEN** (§I rencana): koleksi `inspections`,
`qc_inspections` hantu dicabut (D2), warna beda = **DITAHAN** & handfeel beda =
**PERINGATAN** (keputusan pemilik #5, dikonfirmasi ulang pemilik hari ini), yang
melepas tahanan = **manajer**. Lalu **N (Notifikasi)** → **M (Makloon)**.

---
## §STATUS I — SELESAI (2026-08-24): **INSPEKSI & QC JADI DOKUMEN** · koleksi hantu dicabut · tahanan warna dilepas MANAJER · D2 & D6 lunas

> Ditulis pada sesi penutup (repo `kwakalsaksaia/KN`). Pekerjaan kodenya lahir pada sesi
> sebelumnya; sesi ini **mengukur ulang, menutup gate ULANG, dan membukukannya** — karena
> plan.md sempat berhenti di §STATUS S sehingga seluruh FASE I tidak terbaca di roadmap.

### 0. Titik henti yang diwarisi — DIUKUR, bukan ditebak
Sesi sebelumnya berhenti **di tengah satu perintah** (`gate.sh --full` → alat pemanggil
kena `context deadline exceeded`), bukan di tengah kode.

| Diukur | Angka |
|---|---|
| `/app` saat kontainer datang | **template KOSONG** (`server.py` 88 baris "Hello World") — repo GitHub belum ada di dalamnya |
| `.restore_env.sh` sesudah rsync repo (tanpa `.env`) | HIJAU: `SEED_EXIT=0` · fondasi LENGKAP · `BUILD_EXIT=0` |
| `.emergent/emergent_todos.json` | **17 `completed`** · 1 `in_progress` (gate ULANG) · 1 `pending` (dokumentasi) |
| POC FASE I | **93 PASS / 0 FAIL** |
| `audit_md_erp_readiness` | `SELESAI=92 · BELUM=3 · DRIFT=1` — **BELUM/DRIFT semuanya milik FASE N**, nol milik I |

### 1. Yang dibangun untuk FASE I (rencana §I `RENCANA_EKSEKUSI_MD_ERP.md`)
| Butir rencana | Wujudnya |
|---|---|
| **I.A** D2 koleksi hantu | `qc_inspections` **DICABUT** dari `entity_scope.SCOPED_COLLECTIONS` pada commit yang sama, **ber-alasan tertulis di kode** (`backend/entity_scope.py:123`). Membiarkan nama mati di registry membuat gate "menjaga" sesuatu yang tak ada — dan agen berikutnya percaya koleksinya ada |
| **I.B** koleksi `inspections` | `backend/services/inspection_service.py` **923 baris** · `backend/schemas_inspection.py` · nomor `<ENT>/INS-#####` · SATU koleksi dengan pembeda `kind` (`po_receipt` · `makloon_output` · `return_customer` · `return_supplier` · `replacement`) |
| **I.C** anti pintu ke-3 | Grade **tetap** hanya berubah lewat `grade_service.set_roll_grade(source="qc_inspection")`; `inspections.lines[]` = **ringkasan + keputusan**. Dokumen `po_receipt` **LAHIR OTOMATIS** saat tugas masuk `qc_pending` — melengkapi, bukan menggantikan |
| **I.D** warna & handfeel menentukan | Kebijakan `qc.color_mismatch_action` / `qc.handfeel_mismatch_action` ∈ `abaikan\|peringatkan\|tahan`; **warna beda = DITAHAN**, handfeel beda = **PERINGATAN** (keputusan pemilik #5); roll tertahan **tidak bisa putaway** sampai **MANAJER** melepasnya ber-alasan (`release_hold`) |
| **I.E** endpoint & izin | `backend/routers/inspections.py` **390 baris** (buat · daftar+CSV · detail · assign · start · inspect baris · finish · reopen · PDF · `meta/ref-docs`); resource izin **`inspection`** di `permissions_config` |
| **I.F** milestone retur | `sales_returns` + `shipped_to_store_at` · `shipped_to_customer_at` · `goods_arrived_at` · `inspection_id` · `inspect_done_at` · `complaint_code`; master baru **`complaint_reasons`** (7 baris baku, 7 titik sambung) + garis waktu di `features/sales/ReturnJourneyPanel.jsx`. Terukur: milestone **3/3** dokumen retur demo |
| **I.G** layar | `features/inspections/`: `InspectionsView.jsx` (343) · `InspectionDetailPanel.jsx` (406) · `InspectionFormModal.jsx` (171) · `InspectLineModal.jsx` (215); masuk hub **Gudang & Operasi** lewat `config/hubTabs.js` + `navMeta.js` + `AppViewRouter.jsx` (lazy) |
| **I.H** POC | `backend/test_core_inspeksi_poc.py` **801 baris**, I1–I12 → **93 PASS / 0 FAIL**, nol residu |
| PDF | `pdf_resolvers.resolve_inspection_spk` + tipe dokumen `inspection_spk` ("SPK Inspeksi & QC", **esignable**) lewat platform dokumen G-4 |
| Gate | **`INV-QC-02` dua arah** di `verify_data_integrity`: (a) baris dokumen ber-`grade_after ≠ grade_before` WAJIB punya jejak `grade_history` roll; (b) setiap `grade_history` ber-`source="qc_inspection"` WAJIB punya baris dokumennya — **dengan pengecualian tertulis** untuk grade SAAT ROLL LAHIR (retur karantina), karena itu bukan PERUBAHAN grade |
| Registry | `ENTITY_REGISTRY.md` `### inspections` + `### complaint_reasons` (CHECK 8) |
| Data demo | `seed_realistic.seed_inspections()` — 3 SPK: `KSC/INS-00001` Penerimaan PO *Sedang Diperiksa* dengan **1 baris DITAHAN** · `KSC/INS-00002` Retur Pelanggan *Selesai* · `KANDA/INS-00001` *Draf* |

### 2. Tiga lubang yang hanya terlihat karena layarnya dijalankan sendiri
* **I-FIX-1 — panel mati untuk peran gudang, ditutup di AKARNYA.** Layar memanggil
  `/api/sales-returns` untuk mengisi pemilih dokumen sumber, sementara peran `warehouse`
  memang **tidak punya** `sales_return.view` → pop-up "Buat SPK" lahir **kosong tanpa satu
  pun galat**. Obat yang SALAH adalah melonggarkan izin; yang benar: endpoint sempit
  **`GET /api/inspections/meta/ref-docs`** (izin `inspection`) yang hanya mengembalikan
  yang dibutuhkan pemilih. `audit_sales_roles_ux` HIJAU untuk 7 peran.
* **I-FIX-2 — jalur hasil makloon dulu melahirkan SPK KOSONG.** Nomor (`mko_number`),
  mitra (`steps[].makloon_name`) dan **BARIS** (dari `output_lot_ids`) tak pernah terbaca.
  Diperbaiki dengan **satu definisi** (`makloon_output_lot_ids()`) yang dipakai bersama
  oleh pemilih **dan** pembuat dokumen — supaya keduanya tidak bisa berbeda lagi.
* **I-FIX-3 — pagar entitas + SPK kembar.** `purchase_return` & `makloon_order` ikut
  berpagar badan usaha; SPK kedua **ditolak** selama SPK lama masih berjalan, dan pemilih
  memberi penanda **"sudah ada SPK"** supaya penolakan itu tidak mengejutkan.

### 3. Tiga bug UI nyata (ditemukan dari uji layar sendiri, bukan dari kode)
1. **`KNSelect` melebar menembus pop-up.** Label pilihan yang panjang membuat isi modal
   terpotong di tepi kanan. Akarnya bug flexbox klasik: `<span>` pemicu sudah punya
   `textOverflow: ellipsis` **tanpa `minWidth: 0`**, jadi flex item memakai lebar
   min-content teksnya dan ellipsis **tak pernah berlaku**. Diperbaiki **di komponennya**
   (berlaku untuk SEMUA pemilih aplikasi).
2. **Ambang panjang alasan** (`MIN_REASON = 15`) ditegakkan **server DAN pop-up** — dulu
   tombolnya menyala lalu ditolak backend.
3. **`reject_reason` ikut dikosongkan** saat SPK dibuka kembali (`reopen`) — alasan
   penolakan lama hilang dari dokumen, padahal justru itu jejaknya.

### 4. Bukti penutup (sesi 2026-08-24)
* `gate.sh --full` **HIJAU 2× BERTURUT** — **340 s & 341 s** · receipt **111 gate PASS / 0 FAIL**
* POC FASE I **93/93** · `verify_data_integrity` **241 PASS / 0 FAIL / 0 WARN**
* `audit_doc_refs --strict` HIJAU · `INV-GATE-01` anti-residu HIJAU (stok dipulihkan EKSAK:
  **334 dokumen di 8 koleksi**) · `audit_endpoint_sweep` **472 GET · 5xx = 0**
* `audit_md_erp_readiness --fase I`: **seluruh baris I SELESAI** → utang **D2**
  (`qc_inspections` hantu) dan **D6** (`REL_LABEL` ⊆ `REL_INVERSE`, **18 relasi
  berpasangan**) **LUNAS**
* Alat baru sesi ini: **`.logs/run_gate_bg.sh`** — pelari gate latar belakang ber-kunci
  PID + penanda `.done`, supaya alat ber-timeout tidak bisa lagi menyalakan gate kedua
  (kelas "30+ merah palsu" 2026-08-23)

### 4b. Uji LAYAR sesudah komponen bersama disentuh (sesi 2026-08-24)
Gate hanya membuktikan `KNSelect`/`ConfirmModal`/`FormModal` secara **statik**. Karena
ketiganya dipakai SELURUH aplikasi, perilakunya diuji di peramban — dan sengaja pada
**layar yang TIDAK disentuh FASE I**, karena di situlah risikonya:

| Pemeriksaan | Bukti terukur |
|---|---|
| **BARU-2** SPK hasil makloon berisi baris | `KSC/INS-00003` lahir dari `MKO-00005 · PT Bleaching Pratama` dengan **1 roll · 24,5 yard** (dulu nol baris). Keempat opsi pemilih berlabel `MKO-##### · Nama Mitra` |
| **REGRESI-B** `KNSelect` di layar PO (tak disentuh FASE I) | `supplier-master-select` 6 opsi → terpilih `SUP-00007 · CV Kanda Suka (Entitas grup)` **TERSIMPAN** · `warehouse-select` 4 opsi → `Gudang Bandung Kopo (WH-BDG)` **TERSIMPAN** · **MELUAP=False** (tepi pemicu 954 vs tepi form 1520) |
| **REGRESI-C** `ConfirmModal` di layar Retur Beli | pop-up `return-reject-modal` pada `PRET-00001`: alasan kosong → tombol **disabled**; alasan terisi → **menyala**; ditutup lewat **Batal** supaya data demo tidak berubah |
| **REGRESI-D** Esc satu lapisan (INV-UI-10) | dropdown produk 21 opsi → satu Esc → **opsi 0 · form PO tetap terbuka · supplier tetap terisi** |
| **REGRESI-E** pemilih ber-portal (INV-UI-09) | Wizard Order Makloon, `wizard-material` → pilih *Batik Mega Mendung Premium* → pemilih **tetap tertutup** (diukur ulang 1,8 s kemudian **masih 0**), label pemicu tersimpan, tombol **Lanjut terlihat** |
| Nol residu | `KSC/INS-00003` dihapus lewat `seed_realistic.py`; sesudahnya `verify_data_integrity` **241/0/0** dan `audit_doc_refs --strict` **33/0/0** (nol tautan menggantung) |

**Satu temuan NYATA dari sesi ini — di dokumentasi, bukan di kode.** Agen uji berhenti pada
REGRESI-B/C/E dengan alasan "testid tidak ditemukan". Sebagiannya memang salah cari
(`create-po-button`, `create-po-form`, `supplier-master-select` **ADA**), tetapi satu
alasannya SAH: `memory/test_credentials.md` menyatakan pop-up alasan memakai
`confirm-modal-reason`/`confirm-modal-confirm`, padahal `ConfirmModal` memberi testid
**BERAWALAN** `{testId}-reason`/`{testId}-confirm` dan nama `confirm-modal` hanya berlaku
bila layar tidak mengirim prop `testId`. Pop-up tolak retur beli sesungguhnya
`return-reject-modal-confirm`. Dokumennya sudah dibetulkan beserta cara menemukannya.
Pelajarannya: **dokumen uji yang setengah benar sama mahalnya dengan bug** — ia membuat
pemeriksaan yang paling penting justru dilewati.

### 5. Sisa yang BELUM (jujur, bukan disembunyikan)
Tidak ada sisa milik FASE I. Yang tercatat di `audit_md_erp_readiness` semuanya milik
**FASE N**: `services/notification_audience.py` · `create_notification(recipient_permission=…)` ·
notifikasi PO custom (`special_orders`) · **D5** 11 notifikasi ber-`recipient_role="all"`.
