...
| `gate_residue.py` | ~210 | **INV-GATE-01** | Membuktikan gate tidak mengubah data (sidik jari sebelum/sesudah). **2026-08-21:** daftar pantauan +11 koleksi (`internal_requests` · kas · jurnal · pinjaman & aset antar-PT · eliminasi · PR · `system_settings` · `store_credit_ledger` · `approval_matrix_log` · `credit_overrides`) **dan** pemeriksaan **tautan menggantung** (INV-REF-04) yang TIDAK pernah diabaikan `--ignore-trails` |
| `guardrails/verify_ref_unlink.py` | ~330 | **INV-REF-04** | **BARU 2026-08-21** — penghapus dokumen WAJIB menyapu tautan baliknya. `link()` menulis dua arah, jadi `delete_*` pada 34 koleksi ber-`refs` tanpa `unlink_all()` meninggalkan dokumen hidup yang menunjuk **404**. Statik lewat `ast` (resolusi konstanta lintas-modul `db[cs.COLL]`); `--self-test` 8 kasus dua arah. Bukti-merah pada kode sebelum perbaikan: **5 pelanggaran produksi** |
| `ukur_residu_poc.py` | ~100 | — (alat diagnosis) | **BARU 2026-08-21** — jalankan tiap POC `gate.sh` satu per satu dan hitung SELISIH SELURUH koleksi. Dibuat karena `gate_residue --check` benar tetapi hanya bisa berkata "blok POC meninggalkan residu" tanpa menunjuk POC MANA; menebak dari nama koleksi memakan waktu dan sering salah. Hasil pertama: **14 dari 33 POC** meninggalkan residu |
