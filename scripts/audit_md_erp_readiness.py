...
def kode_saja(src: str) -> str:
    """Buang komentar `#…` supaya PENGUKURAN membaca KODE, bukan prosa tentang kode.

    Pelajaran mahal 2026-08-23 (FASE I): `qc_inspections` sudah DICABUT dari
    `entity_scope.SCOPED_COLLECTIONS`, tetapi commit yang mencabutnya meninggalkan
    komentar alasan — persis seperti yang diminta rencana §I.A. Alat ini mengukur
    dengan `"qc_inspections" in <teks berkas>`, jadi ia membaca KOMENTAR-nya dan
    melaporkan DRIFT yang tidak ada. Satu jam dipakai memburu hantu.

    Karena itu: setiap pengukuran keanggotaan daftar dilakukan pada sumber TANPA
    komentar. Alat ukur yang bisa tertipu oleh dokumentasi bukan alat ukur.
    """
    keluar: List[str] = []
    for baris in src.splitlines():
        ...
        if tanda == -1:
            keluar.append(baris)
            continue
        # `#` di dalam string (mis. warna "#fff") jangan dianggap komentar.
        if baris[:tanda].count('"') % 2 == 0 and baris[:tanda].count("'") % 2 == 0:
            keluar.append(baris[:tanda])
        else:
            keluar.append(baris)
    return "\n".join(keluar)
...
SRC = {
    ...
    # Sama, tetapi TANPA komentar — dipakai untuk semua pengukuran keanggotaan daftar
    # (lihat `kode_saja`). Keduanya disimpan karena beberapa pemeriksaan memang ingin
    # membaca alasan yang tertulis, bukan hanya kodenya.
    "entity_scope_kode": kode_saja(read(BE / "entity_scope.py")),
...
def cek_pagar_entitas(existing: Dict[str, int]) -> None:
    for coll, meta in NEW_COLLECTIONS.items():
        ...
        scoped = bool(re.search(rf'"{coll}"', SRC["entity_scope_kode"]))
        ...
        if kind == "master":
            inh = bool(re.search(rf'"{coll}":\s*\[', SRC["entity_scope_kode"]))
...
async def cek_fase_i(db, existing) -> None:
    # Diukur pada KODE (tanpa komentar): commit yang MENCABUT `qc_inspections`
    # meninggalkan komentar alasan — dan itu memang diminta rencana §I.A. Mengukur
    # teks mentah membuat dokumentasi terbaca sebagai drift (lihat `kode_saja`).
    hantu = ("qc_inspections" in SRC["entity_scope_kode"]
             and "qc_inspections" not in existing)
    add("I", DRIFT if hantu else SELESAI,
        ...
        if hantu
        else "DICABUT dari entity_scope (FASE I) — namanya diganti `inspections` "
             "karena cakupannya lebih luas dari QC; alasannya tertulis di kode")
...
async def cek_fase_pdnm(db, existing) -> None:
    ...
    add("P", DRIFT if po_pr == 0 else SELESAI, "rantai PO → PR → SO untuk `sales_name`",
        ...
        f"menyimpan pr_id · PR.source={src}" +
        (" (TIDAK ADA) → FASE P wajib membuat tautannya DULU, kalau tidak "
         "`sales_name` mustahil dirunut" if po_pr == 0 else
         " — P-0 SELESAI 2026-08-21: `so_repeat` sudah ada di data demo, tautan "
         "dua arah ditulis di titik lahir PO (dijaga INV-ORIG-01). PO lama SENGAJA "
         "dibiarkan tanpa `pr_id` (keputusan pemilik: tanpa backfill), jadi angka "
         "ini TIDAK akan pernah 100% — dan itu jujur, bukan cacat."))
    ...
    # KENAPA BUKAN `recipient_permission=` (bentuk yang diusulkan rencana v2): argumen
    # itu SENGAJA tidak dibuat. Penyaring pembaca memakai OR (`recipient_role ∈ {peran,
    # "all"}` OR `recipient_user`), jadi menyimpan "izin" di dokumen notifikasi tetap
    # akan MENYIARKAN begitu `recipient_role` ikut terisi. Bentuk yang benar — dan yang
    # dipakai 7 produsen notifikasi hari ini — adalah FAN-OUT PER ORANG lewat
    # `create_addressed(permission=…)` (`ref` berakhiran `#<user_id>` supaya dedupe
    # berjalan per orang). Fakta ini mengukur API yang NYATA, bukan nama yang batal.
    perm_arg = ("async def create_addressed" in SRC["notif"]
                and "permission=permission" in SRC["notif"])
    add("N", SELESAI if perm_arg else BELUM, "create_addressed(permission=…) — alamat dari WEWENANG",
        "ada (fan-out per orang; `recipient_permission` sengaja tidak dipakai — "
        "penyaring pembaca ber-OR akan menyiarkannya)"
        if perm_arg else "WAJIB: hari ini hanya recipient_role / recipient_user")
