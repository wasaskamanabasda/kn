...
# FASE E-8 (E8.1) — peran baru TIDAK ditulis ke `roles:` di navStructure.js;
# visibilitas menunya dinyatakan sekali di `config/roles.js` (ROLE_NAV:
# inherit/add/remove) dan gate ini membaca overlay itu — supaya gate menilai navigasi
# yang BENAR-BENAR dirender, bukan separuhnya.
#
# FASE D (2026-08-20) — DAFTAR PERAN TIDAK BOLEH DI-HARDCODE DI SINI LAGI.
# Sebelumnya baris ini berbunyi:
#     ROLES = ["admin", "sales", "manager", "warehouse", "sales_admin", "finance"]
# yaitu SUMBER KEDUA untuk "peran apa saja yang ada". Begitu peran ke-7 (`designer`)
# lahir di `config/roles.js` + `navMeta.ROLE_HOME_REGISTRY`, `check_roles()` membaca
# beranda peran dari SSOT tetapi mencari keterjangkauannya di `reach` yang dibangun
# dari daftar basi ini → gate **CRASH** `KeyError: 'designer'`. Gejalanya bukan
# "gate memerah dengan penjelasan", melainkan traceback: paling mahal, karena gate
# yang meledak tidak memeriksa APA PUN sesudah baris itu.
#
# Sekarang daftar peran DIBACA dari `ROLE_REGISTRY` (SSOT sisi layar, cermin
# `backend/role_registry.py`). Peran ke-8 kelak tidak butuh menyentuh berkas ini.
...
def parse_role_ids():
    """Daftar peran DARI SSOT `config/roles.js` → `ROLE_REGISTRY`, urut `order`.

    FASE D: menggantikan konstanta `ROLES = [...6 nama...]` yang dulu di-hardcode di
    berkas ini. Alasannya sudah dibuktikan sendiri oleh repo ini: peran ke-7
    (`designer`) lahir di SSOT, `parse_role_home()` membacanya, tetapi `reach` masih
    dibangun dari daftar basi → `KeyError: 'designer'` dan gate MELEDAK sebelum
    memeriksa apa pun.

    Sengaja MELEDAK BERISIK (SystemExit) bila blok registry tak bisa dibaca: kembali
    ke daftar cadangan yang di-hardcode berarti menghidupkan ulang sumber kedua —
    gate akan hijau sambil tidak memeriksa peran yang sebenarnya ada.
    """
    if not ROLES_CFG.exists():
        print(f"{C.RED}✗ SSOT peran tidak ditemukan: {ROLES_CFG}{C.RESET}")
        sys.exit(2)
    src = ROLES_CFG.read_text()
    m = re.search(r"export const ROLE_REGISTRY\s*=\s*\{(.*?)\n\};", src, re.S)
    if not m:
        print(f"{C.RED}✗ blok `ROLE_REGISTRY` tak terbaca di {ROLES_CFG.name} — "
              f"gate menolak memakai daftar peran cadangan (itu sumber kedua).{C.RESET}")
        sys.exit(2)
    body = m.group(1)
    found = []
    # Hanya entri pada indentasi 2 spasi = anggota langsung registry (bukan nested).
    for rm in re.finditer(r"^  (\w+):\s*\{(.*?)^  \},", body, re.S | re.M):
        rid, blk = rm.group(1), rm.group(2)
        order = re.search(r"order:\s*(\d+)", blk)
        found.append((int(order.group(1)) if order else 999, rid))
    if not found:
        print(f"{C.RED}✗ `ROLE_REGISTRY` terbaca tetapi 0 peran ditemukan.{C.RESET}")
        sys.exit(2)
    return [rid for _, rid in sorted(found)]


#: Diisi di `main()` dari SSOT — bukan konstanta yang ditulis tangan.
ROLES = parse_role_ids()
