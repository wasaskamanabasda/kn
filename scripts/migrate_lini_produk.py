...
SEED_LINES: List[Dict[str, Any]] = [
    {"code": "woven", "name": "Woven (Tenun)", "sort": 1, "active": True,
     ...
     # FASE P — kode TAHAP PROSES (`process_stages`), bukan tahap BAHAN.
     # `yarn` (tahap bahan) diganti `benang` (langkah kerja); baris lama
     # dinormalkan `normalize_stage_sequence()` di bawah. Dijaga `INV-STAGE-01`.
...
#: FASE P — kode tahap LAMA yang sebenarnya milik kosakata **tahap bahan**
#: (`domain_registry.MATERIAL_STAGES`), dipetakan ke **tahap proses** yang setara
#: (`process_stages`). Peta ini SENGAJA eksplisit dan pendek: menormalkan kode
#: yang tak dikenal dengan menebak (mis. mencocokkan awalan huruf) akan
#: memindahkan pekerjaan ke tahap yang salah tanpa ada yang tahu.
STAGE_ALIAS = {"yarn": "benang"}


async def normalize_stage_sequence(db, dry: bool) -> Tuple[int, List[str]]:
    """Samakan `product_lines.stage_sequence` dengan master `process_stages`.

    KENAPA LANGKAH INI ADA (ditemukan FASE P, 2026-08-21)
    ----------------------------------------------------
    Papan PO membaca urutan tahap dari lini, lalu mengambil LABEL-nya dari master
    tahap. Baris `woven`/`knit` menyimpan `yarn` — kode yang **tidak ada** di
    `process_stages` (di sana namanya `benang`, "Benang (bahan masuk)"). Gejalanya
    bukan galat: papan menampilkan chip **"Yarn"** hasil menebak dari kodenya,
    sehingga master tahap yang diubah pemilik (nama, urutan) **tidak berpengaruh
    apa pun** pada tahap pertama — dan tidak ada yang tahu kenapa.

    → (jumlah baris diperbaiki, daftar kode asing yang TIDAK punya alias)
    """
    known = {str(s.get("code") or "").strip().lower()
             async for s in db.process_stages.find({}, {"_id": 0, "code": 1})}
    fixed, unknown = 0, []
    async for line in db.product_lines.find({}, {"_id": 0, "id": 1, "code": 1,
                                                 "stage_sequence": 1}):
        seq = [str(s or "").strip().lower() for s in (line.get("stage_sequence") or [])]
        if not seq:
            continue
        ...
        for code in seq:
            if code in known:
                new_seq.append(code)
                continue
            alias = STAGE_ALIAS.get(code)
            if alias and alias in known:
                new_seq.append(alias)
                changed = True
                continue
            # Tidak dikenal & tanpa alias: DIBIARKAN (jangan hilangkan tahap yang
            # mungkin memang tahap baru pemilik) tetapi DILAPORKAN, supaya gate
            # `INV-STAGE-01` dan orang yang membaca laporan tahu harus berbuat apa.
            new_seq.append(code)
            unknown.append(f"{line.get('code')}:{code}")
        if changed and not dry:
            await db.product_lines.update_one(
                {"id": line["id"]},
                {"$set": {"stage_sequence": new_seq, "updated_at": _now()}})
        fixed += 1 if changed else 0
    return fixed, sorted(set(unknown))
...
async def backfill_documents(db, dry: bool,
    ...
    yang memuat jenis `proofing` diberi lini `printing`, karena proofing memang
    ...
    FASE S: dibaca dari `sample_types[]` **dan** `sample_type` lama, supaya migrasi
    ini tetap benar dijalankan sebelum maupun sesudah `migrate_sample_types.py`.
    """
    ...
    async for smp in db.md_samples.find({}, {"_id": 0, "id": 1, "line_code": 1,
                                             "sample_types": 1, "sample_type": 1,
                                             "product_id": 1, "spec_id": 1}):
        if str(smp.get("line_code") or "").strip():
            continue
        if prod_line.get(smp.get("product_id")):
            continue
        kinds = [str(v).strip().lower() for v in (smp.get("sample_types") or [])]
        if not kinds and smp.get("sample_type"):
            kinds = [str(smp["sample_type"]).strip().lower()]
        if "proofing" not in kinds:
            continue
        extra += 1
        if not dry:
...
async def main(dry: bool) -> int:
    ...
    n_fix, unknown = await normalize_stage_sequence(db, dry)
    print(f"\n1b) FASE P — `stage_sequence` disamakan dengan master tahap: "
          f"{n_fix} lini diperbaiki (alias {STAGE_ALIAS})")
    if unknown:
        print(f"   ⚠ kode tahap ASING (tidak ada di `process_stages`, tanpa alias): "
              f"{unknown} → tambahkan tahapnya di Pengaturan → Master → Tahapan Proses, "
              "atau perbaiki urutan lininya. Gate INV-STAGE-01 akan memerah sampai beres.")
