...
async def _create(title: str, *, actor: str, due_offset: int, supplier_ids: list,
                  sample_types: list = None, brief: str = "", design_id: str = "",
                  qty: float = 3, unit: str = "meter", sla_days: int = 7,
                  batch: str = BATCH) -> dict:
    payload = {
        # FASE S — DAFTAR jenis (bukan satu kata). Demo KPI sengaja tetap SATU jenis
        # per permintaan supaya angka rapor mudah dibaca; permintaan dua-jenis
        # diperagakan `seed_realistic.py::seed_rnd()`.
        "sample_types": list(sample_types or ["labdip"]), "title": title,
...
async def _round(sample_id: str, supplier_id: str, *, actor: str, note: str,
                 ...
                 assess_note: str = "", assessor: dict = None,
                 type_code: str = "") -> dict:
    ...
    FASE S — `type_code` menyaring round bila permintaan menempuh >1 jenis; kosong
    berarti "ambil round terbuka apa pun" (aman karena demo ini satu jenis).
    """
    doc = await smp.get_sample(sample_id)
    mine = [r for r in (doc.get("rounds") or [])
            if r["supplier_id"] == supplier_id and r["status"] == "open"
            and (not type_code or str(r.get("type_code") or "") == type_code)]
    if not mine:
...
async def seed_me(verbose: bool = True) -> int:
    ...
    await _round(a["id"], s1["id"], actor=ME,
                 ...
                 meas={"delta_e": 0.8, "colorfastness_wash": 4, "colorfastness_rub": 4},
    ...
    await _round(a["id"], s2["id"], actor=ME,
                 ...
                 meas={"delta_e": 2.2, "colorfastness_wash": 3, "colorfastness_rub": 3},
...
async def seed(verbose: bool = True) -> int:
    ...
    await _round(a["id"], s1["id"], actor=RINA,
                 ...
                 meas={"delta_e": 1.1, "colorfastness_wash": 4, "colorfastness_rub": 4},
    ...
    await _round(a["id"], s2["id"], actor=RINA,
                 ...
                 meas={"delta_e": 2.8, "colorfastness_wash": 3, "colorfastness_rub": 3},
    ...
    await _round(a["id"], s2["id"], actor=RINA,
                 ...
                 meas={"delta_e": 1.4, "colorfastness_wash": 4, "colorfastness_rub": 4},
    ...
    stype = "proofing" if design else "labdip"   # FASE S — dikirim sebagai DAFTAR
    c = await _create("Proofing Rayon Motif Kawung Klasik" if design
                      ...
                      sample_types=[stype], design_id=(design or {}).get("id", ""),
    ...
    await _round(c["id"], s1["id"], actor=BAGAS,
                 ...
                 meas={"delta_e": 4.2, "repeat_cm": 33, "register_mm": 2.5},
    ...
    await _round(c["id"], s2["id"], actor=BAGAS,
                 ...
                 meas={"delta_e": 2.6, "repeat_cm": 32, "register_mm": 0.8},
...
async def seed_trend_history(verbose: bool = True) -> int:
    ...
    pada grafik tren.
    """
    from core_utils import new_id, next_doc_number, now_iso  # noqa: PLC0415
        ...
        for idx, rounds_spec in enumerate(months):
            ...
            for rno, (result, on_time, score) in enumerate(rounds_spec, start=1):
                ...
                rounds.append({
                    ...
                    # FASE S — round WAJIB menyebut jenisnya sendiri; tanpa ini
                    # round historis tidak muncul di rangkaian mana pun begitu
                    # satu permintaan menempuh lebih dari satu jenis.
                    "type_code": "labdip", "qc": {},
            ...
            docs.append({
                "id": new_id("smp"),
                # ── D7 DIPERBAIKI (FASE S) ────────────────────────────────────
                # Dulu: f"KSC/SMP-H{back}{designer[:2].upper()}" — penomoran sendiri
                # dengan f-string. Akibatnya TERUKUR: 20 dari 28 nomor menyimpang
                # dari pola `<ENT>/SMP-#####`, dan dua desainer ber-awalan huruf
                # sama (Dewi Lestari & Dewi Rahayu) menghasilkan **NOMOR DOKUMEN
                # KEMBAR** (`KSC/SMP-H1DE` dst, 5 pasang). Nomor dokumen kembar
                # membuat papan & laporan menampilkan dua baris bernomor sama, dan
                # gate keunikan nomor apa pun akan memerah dengan benar.
                # Sekarang: satu-satunya penomor dokumen di repo ini.
                "number": await next_doc_number("md_samples", "number", "SMP-",
                                                entity_id=ENT),
                "title": f"Riwayat KPI {designer} — {anchor.strftime('%b %Y')}",
                "sample_types": ["labdip"], "status": "decided",
                ...
                # FASE S — penanda pelaksanaan ADA sebagai field tetapi KOSONG:
                # sample historis ini tidak punya catatan jadi/kirim yang nyata, dan
                # mengarang tanggalnya akan membuat laporan pengiriman sample palsu.
                # Field yang ada dengan nilai kosong = "belum tercatat"; field yang
                # tidak ada = layar tidak bisa membedakan "belum" dari "tak ada".
