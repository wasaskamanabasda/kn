...
frontend:
  ...
  - task: "FASE D — PERMINTAAN DESAIN (DSR) + peran ke-7 `designer`: 9 gate merah warisan ditutup"
    implemented: true
    working: true
    file: "backend/routers/design_requests.py, backend/services/design_request_service.py, frontend/src/features/design/*, scripts/check_nav_map.py, scripts/guardrails/verify_approval_queues.py, scripts/guardrails/verify_home_kpi.py, scripts/audit_sales_roles_ux.py, ENTITY_REGISTRY.md"
    needs_retesting: true
    priority: "high"
    stuck_count: 0
    status_history:
        - working: false
          agent: "main"
          comment: "Titik henti warisan DIUKUR (bukan ditebak): FASE D terbangun sebagian (koleksi design_requests 4 dok, service, router, 4 layar, akun designer@, izin, nav hub) tetapi gate.sh --full MERAH 9 dari 120. Akar dikelompokkan: (A) peran ke-7 lahir tanpa didaftarkan di tempat yang MENGHITUNG peran -> check_nav_map.py CRASH KeyError 'designer' (ROLES di-hardcode 6 nama) + POC E-8 G1 mengunci angka 6 di 6 asersi + audit_sales_roles_ux menemukan 3 PANEL MATI milik designer; (B) pintu keputusan design_request tidak terdaftar di penjaga -> INV-APPR-01 & INV-HOME-01 merah, merembes ke POC F-6 (4 gagal) & F-6.7 (2 gagal); (C) 2 gagal yang TERBUKTI bukan bug kode."
        - working: true
          agent: "main"
          comment: "SEMUA DITUTUP. (A1) check_nav_map.py membaca daftar peran dari SSOT frontend/src/config/roles.js ROLE_REGISTRY dan MELEDAK BERISIK bila blok itu tak terbaca (menolak daftar cadangan = menolak sumber kedua). (A2) POC E-8 G1 + role_registry --self-test kini menguji PARITAS server<->layar, bukan populasi peran: 64/64 & 'HIJAU - 7 peran, 0 gagal'. (A3) 2 dari 3 panel mati diperbaiki DI AKARNYA: /suppliers & /warehouses menumpang di modul bersama features/rnd/rndApi.js yang diimpor layar milik designer padahal tak pernah dipanggil (listWarehouses NOL PEMAKAI = mati; listSuppliers dipindah ke satu-satunya pemakainya SampleSendModal.jsx). Panel ke-3 disembuhkan dengan endpoint BARU GET /api/design/reports/mine (izin design_request.view, bukan .report) + tab 'Rapor Saya' -> designer melihat angka DIRINYA tanpa satu nama rekan pun (pola PS-18). Hanya 2 baris pembebasan ditambah ke TERGATED_DI_KODE, keduanya pagar canCreate/canReport yang NYATA. (B1) DOOR_QUEUE + (B2) verify_home_kpi.EXPECT diisi design_request (status delivered; revision SENGAJA tidak dihitung karena bola sudah kembali ke desainer) -> 221 & 80 cek hijau; POC F-6 43/43, F-6.7 41/41 (kegagalan 'harga langganan' DIVERIFIKASI sebagai rembesan, bukan diasumsikan). (C) POC E-4 'Kategori Biaya 0 baris global' + POC E-8 G2/G3 'hr/visits/mine 404' TERBUKTI artefak lingkungan: koleksi fondasi (expense_categories, gl_accounts, hr_employees) hanya lahir dari bootstrap.run_bootstrap() saat backend START, dan DB sempat di-drop tanpa restart -> keduanya hijau tanpa satu baris kode aplikasi diubah (E-4 56/0, E-8 desk 97/97). .restore_env.sh dikeraskan: mongodb dipastikan HIDUP sebelum backend di-restart + langkah VERIFIKASI FONDASI yang gagal-berisik. BUKTI: gate.sh --full HIJAU 2x berturut (309s & 311s, 145 PASS) - verify_data_integrity 237/0/0 - validate_compliance 22 PASS/0 FAIL - audit_md_erp_readiness --fase D SEMUA SELESAI - build FE bersih."
  - task: "POC BARU FASE D (rencana MD-ERP paragraf D.D) — backend/test_core_design_request_poc.py"
    implemented: true
    working: true
    file: "backend/test_core_design_request_poc.py"
    needs_retesting: false
    priority: "high"
    stuck_count: 0
    status_history:
        - working: true
          agent: "main"
          comment: "55 PASS / 0 FAIL, dijalankan 4x berturut dengan NOL RESIDU (idempotent). Membuktikan D1 alur ujung-ke-ujung, D2 alasan revisi WAJIB di server, D3 serah hasil harus menunjuk artwork NYATA di badan usaha yang sama, D4 rapor == hitung-ulang MANDIRI dari MongoDB, D5 masuk antrean keputusan + gate INV-APPR-01/INV-HOME-01 dijalankan SUNGGUHAN, D6 refs DUA ARAH (parent/child di refs[] kedua dokumen - koreksi asumsi awal saya yang mencari koleksi doc_relations), D7 mode gabungan 409, D8 peran ke-7 sempit tapi jujur (IDOR 403, tak boleh assign/approve/reject/create, rapor rekan 403, rapor SAYA 200 tanpa nama rekan), D9 gate MEMBACA peran ke-7 dari registry, D10 nol residu DIUKUR (termasuk mencabut refs yang ditanam di pesanan demo + jejak login). Terdaftar di scripts/gate.sh dengan label 'POC FASE D - PERMINTAAN DESAIN (DSR)' supaya tidak tertukar dengan 'POC FASE D (makloon rantai proses)' dari penomoran lama."

  - task: "FASE I — INSPEKSI & QC SEBAGAI DOKUMEN: panel mati gudang ditutup di akarnya + jalur hasil makloon diperbaiki"
    implemented: true
    working: true
    file: "backend/routers/inspections.py, backend/services/inspection_service.py, frontend/src/features/inspections/inspectionsApi.js, frontend/src/features/inspections/InspectionFormModal.jsx, frontend/src/components/KNSelect.jsx, backend/test_core_inspeksi_poc.py"
    needs_retesting: true
    priority: "high"
    stuck_count: 0
    status_history:
        - working: false
          agent: "main"
          comment: "TITIK HENTI WARISAN DIUKUR (bukan ditebak). Commit WIP berhenti tepat sesudah endpoint `GET /api/inspections/meta/ref-docs` ditambahkan, tetapi LAYAR-nya belum dipindah: `inspectionsApi.refDocOptions()` masih memanggil `/api/sales-returns` langsung. Bukti-merah: (a) `audit_sales_roles_ux` = '1 LAYAR/PANEL MATI — warehouse → inspections → /sales-returns'; (b) login gudang → GET /api/sales-returns = 403 'Permission ditolak: sales_return.view' sementara GET /api/inspections/meta/ref-docs?kind=return_customer = 200 (2 pilihan). CATATAN JUJUR: panel mati itu terdeteksi STRUKTURAL (penilaian penutupan impor) — tombol 'Buat SPK Inspeksi' sendiri memang di-gate `canCreate=['admin','manager']`, jadi pengguna gudang tidak pernah membuka pop-upnya. Kerusakan yang BENAR-BENAR terlihat pengguna ada di jalur hasil makloon (lihat di bawah)."
        - working: false
          agent: "main"
          comment: "TIGA CACAT NYATA di jalur `makloon_output` yang ditemukan saat memindahkan pemilih (semuanya DIUKUR lewat API, bukan dibaca dari kode): `create()` membaca `mko['number']` (tak ada — nomornya `mko_number`), `mko['makloon_name']` (tak ada di akar — hidup di `steps[]`, mitra bisa beda per tahap), dan mencari roll lewat `inventory_rolls.makloon_order_id` (field itu TIDAK ADA: 0 dari 66 roll; penaut nyatanya `steps[].output_lot_ids` → `inventory_rolls.lot_id`). Akibatnya SPK hasil makloon lahir TANPA nomor acuan, TANPA nama mitra, dan NOL BARIS — dokumen yang tidak bisa diperiksa siapa pun, dan tidak ada satu galat pun yang memberi tahu (terukur: KSC/INS-00005 lahir dengan lines=[])."
        - working: true
          agent: "main"
          comment: "SEMUA DITUTUP DI AKARNYA. (1) Layar memakai `/api/inspections/meta/ref-docs` → `audit_sales_roles_ux` HIJAU (7 peran, nol panel mati) DAN izin `sales_return.view` TETAP tertutup untuk gudang (POC menjaga keduanya, karena 'obat' yang salah untuk panel mati adalah melonggarkan izin). (2) Tiga helper SATU DEFINISI di `inspection_service` (`makloon_ref_number`, `makloon_output_lot_ids`, `makloon_vendor`) dipakai BERSAMA oleh pemilih dan pembuat dokumen → label di pemilih sama dengan isi dokumen ('MKO-00004 · PT Rotary Print Indah', 5 baris roll). (3) Order makloon yang belum menyerahkan hasil TIDAK ditawarkan lagi (MKO-00002 `in_process`) dan ditolak 400 ber-kalimat menuntun bila dipaksa lewat API. (4) Pagar entitas ditambah untuk `purchase_return` & `makloon_order` (sebelumnya hanya `sales_return` yang punya) → dokumen PT lain ditolak meski id-nya ditebak. (5) SPK KEDUA atas dokumen yang sama DITOLAK selama SPK lama masih berjalan, dan pemilih MENANDAI dokumen yang sudah ber-SPK ('sudah ada SPK KSC/INS-00002 (sudah diputuskan)') + catatan sebaris di pop-up → penolakan bisa diduga sebelum tombol ditekan. (6) `attach_inspection` kini HANYA untuk `return_customer`: SPK `replacement` memeriksa barang PENGGANTI, jadi menuliskannya ke `sales_returns.inspection_id` akan menggeser garis waktu retur ke dokumen yang bukan pemeriksa returnya. BUKTI: POC FASE I 73→91 PASS/0 FAIL (18 pemeriksaan I12 baru, nol residu) · gate.sh --full HIJAU 2x berturut (341s & 345s, 111 gate PASS/0 FAIL) · verify_data_integrity 241/0/0 · eslint 0 error · build FE OK."
  - task: "Akar 11 gate merah warisan: POC E-9 & F-1 meninggalkan SPK inspeksi otomatis (residu → tautan menggantung)"
    implemented: true
    working: true
    file: "backend/test_core_rantai_retur_poc.py, backend/test_fase_f1_receiving_uom_poc.py, scripts/audit_doc_refs.py"
    needs_retesting: false
    priority: "high"
    stuck_count: 0
    status_history:
        - working: false
          agent: "main"
          comment: "`gate.sh --full` MERAH 11 gate: POC P-0, POC FASE P, INV-GATE-01 anti-residu, lalu POC G-4/G-2/G-3/F/G-8/G-9/G-7/G-6b. Yang PERTAMA merah cuma satu sebabnya: 'Tautan menggantung: 1 temuan' + \"koleksi 'inspections': 3 -> 4 dok\". Sepuluh sisanya GEMA (masing-masing gagal di pemeriksaan 'keadaan awal invarian HIJAU')."
        - working: true
          agent: "main"
          comment: "AKAR: FASE I melahirkan SPK `po_receipt` OTOMATIS saat barang PO masuk antrean QC (`ensure_for_qc_task`) — dan dua POC yang menerima barang PO tidak tahu itu, jadi SPK-nya tertinggal; ref SPK→PO (PO-nya mereka hapus) menjadi tautan menggantung yang memerahkan gate BERIKUTNYA. Pelakunya ditemukan dengan BUKTI, bukan dugaan: pemantau `.logs/_pantau_residu_inspeksi.py` berjalan bersamaan gate dan mencatat 'LAHIR <nomor> … | GATE SAAT ITU: <nama gate>' → POC FASE E-9 (KANDA/INS-00002) dan POC FASE F-1 (KSC/PO-00024). Keduanya kini menghapus SPK yang lahir dari pekerjaannya + mencabut ref di `md_samples`; diukur standalone 3→3 dan tetap HIJAU. Sekalian: laporan `audit_doc_refs` menyebut NOMOR dokumen ('KANDA/INS-00002 (inspection:ins_…) —parent→ … [HILANG]') karena `dangling_refs()` sudah menghitung `from_number`/`to_number` tetapi versi lama membuangnya — diagnosis ini menelan dua kali `gate --full`, sesi berikutnya tidak perlu mengulanginya."
  - task: "Bug UI nyata di komponen bersama: KNSelect combobox MELEBAR menembus pop-up (ellipsis tak pernah berlaku)"
    implemented: true
    working: true
    file: "frontend/src/components/KNSelect.jsx, frontend/src/features/inspections/InspectionFormModal.jsx"
    needs_retesting: true
    priority: "medium"
    stuck_count: 0
    status_history:
        - working: true
          agent: "main"
          comment: "Ditemukan saat verifikasi layar SENDIRI (bukan dari kode): label pilihan panjang ('SRET-00001 · Toko Kain Sejahtera · Warna tidak sesuai · sudah ada SPK KSC/INS-00002 (sudah diputuskan)') membuat isi pop-up terpotong di tepi kanan. Akarnya bug flexbox klasik: `<span>` pemicu sudah punya `textOverflow: ellipsis` TAPI tanpa `minWidth: 0`, sehingga flex item memakai lebar min-content teksnya dan tombolnya MELEBAR — ellipsis tak pernah berlaku. Diperbaiki di komponennya (berlaku untuk SEMUA pemilih di aplikasi) + `min-w-0` pada grid & label pop-up. Diukur ulang di peramban: tepi kanan pemilih 1280 vs tepi modal 1296 → MELUAP=False, teks ter-ellipsis."

metadata:
  created_by: "main_agent"
  version: "FASE-I-CLOSED-2026-08-24"
  test_sequence: 240
  run_ui: true

test_plan:
  current_focus:
    - "I.1 — kepala gudang: barang PO diterima → SPK Inspeksi OTOMATIS ada, hanya perlu menugaskan petugas"
    - "I.2 — petugas inspect: isi cacat per roll + hasil WARNA & HANDFEEL vs sample yang di-ACC; grade muncul otomatis dari poin"
    - "I.3 — warna beda dari sample → roll DITAHAN dan layar menyebut siapa yang berwenang melepas"
    - "I.4 — manajer menolak inspeksi: alasan WAJIB, dan alasannya terlihat DI DOKUMEN"
    - "I.5 — dari dokumen inspeksi bisa melompat ke PO/roll/sample acuan (jejak dua arah)"
    - "BARU: pop-up Buat SPK — pemilih dokumen sumber (retur pelanggan · retur supplier · hasil makloon) + penanda 'sudah ada SPK'"
    - "BARU: SPK hasil makloon berisi baris roll + nomor MKO + nama mitra (dulu lahir kosong)"
  stuck_tasks: []
  test_all: false
  test_priority: "high_first"
...
agent_communication:
    - agent: "main"
      message: "FASE I (SPK INSPEKSI & QC) SIAP DIUJI LEWAT LAYAR. Keadaan terukur ULANG pada sesi penutup 2026-08-24 (sesudah lingkungan dipulihkan dari repo kwakalsaksaia/KN): gate.sh --full HIJAU 2x berturut (340s & 341s · 111 gate PASS / 0 FAIL) · POC FASE I 93 PASS/0 FAIL (nol residu) · verify_data_integrity 241 PASS/0 FAIL/0 WARN · audit_endpoint_sweep 472 GET · 5xx=0 · audit_md_erp_readiness --fase I SELURUH baris I SELESAI · audit_sales_roles_ux HIJAU (7 peran, nol panel mati) · eslint 0 error · build FE OK. DATA DEMO: 3 dokumen inspeksi (KSC/INS-00001 Penerimaan PO `Sedang Diperiksa` 1 baris DITAHAN · KSC/INS-00002 Retur Pelanggan `Selesai` · KANDA/INS-00001 Draf), 3 retur jual, 5 order makloon, 7 alasan keluhan, 28 sample. CARA MASUK: `login-email-input` / `login-password-input` / `login-submit-button`, sandi `demo12345` untuk SEMUA akun (admin@kainnusantara.id, manager@kainnusantara.id, warehouse@kainnusantara.id). SESUDAH masuk WAJIB pilih badan usaha: klik pita `scope-pick-ent_ksc` (mode 'Semua Entitas' sengaja HANYA-LIHAT, aksi tulis dijawab 409 — itu BUKAN bug). JALAN KE LAYAR (navigasi BUKAN hash-routing, jangan pakai URL /#/...): klik `nav-group-gudang` → `nav-wms-operations` → tab hub `hub-tab-inspections`. Untuk akun gudang, layar hub itu sudah menjadi beranda perannya sehingga `hub-tab-inspections` bisa langsung diklik. TESTID PENTING: inspections-view, ins-kpi-total/draft/progress/hold/rejected, ins-policy-note, ins-search, ins-refresh, ins-chip-kind-{kind}, ins-chip-status-{status}, ins-chip-hold, ins-row-{id}, ins-detail-{id}, ins-detail-panel, ins-detail-number/status/baseline/reject-reason/history, ins-line-{lineId}, ins-line-inspect-{lineId}, ins-line-release-{lineId}, ins-line-hold-{lineId}, ins-assign-select, ins-assign-bagian, ins-assign-button, ins-start-button, ins-decision-select, ins-finish-button, ins-reopen-button, ins-create-button, ins-create-modal, ins-kind-select, ins-ref-select, ins-ref-spk-open, ins-ref-spk-done, ins-assignee-select, ins-remark-input, ins-create-submit, ins-color-select, ins-handfeel-select, ins-handfeel-score, ins-delta-e, ins-gsm-actual, ins-width-actual, ins-defect-{kodeCacat}, ins-points-preview, ins-will-hold-warning, ins-line-remark. WAJIB DIBACA — `KNSelect` merender placeholder SEBAGAI OPSI (`{testId}-option-empty`); memilihnya MENGOSONGKAN pilihan sehingga tombol simpan mati. Selalu pakai testid kanonik `{testId}-option-{value}`, JANGAN `[role=option]` generik dan JANGAN `text=...` (teks yang sama sering ada di tabel di belakang modal — itu sudah membuat satu klik saya gagal). Pop-up ALASAN WAJIB memakai `confirm-modal`: isi `confirm-modal-reason` lalu `confirm-modal-confirm`; tombol itu DISABLED selama alasan kosong/terlalu pendek (minimal 15 huruf) — DISENGAJA, bukan bug. YANG SENGAJA TIDAK ADA (jangan laporkan sebagai bug): untuk peran `warehouse` tidak ada `ins-create-button` (gudang boleh memeriksa, tidak menerbitkan SPK) dan tidak ada `ins-line-release-*` (pelepas tahanan hanya manajer/admin — keputusan pemilik #5); tombol `ins-finish-button`/`ins-reopen-button` hanya untuk admin/manajer. JANGAN UJI: drag-and-drop, unggah berkas/kamera, suara. JANGAN laporkan 'NaN' hasil pencarian kata Indonesia ber-'nan' (mis. 'Penanganan') — dua temuan seperti itu sudah terbukti PALSU. WAJIB: reproduksi dulu sebelum melaporkan; sebutkan langkah + testid; dan PERIKSA RESIDU dokumen yang Anda buat (pemulihan: `python seed_realistic.py` lalu `python seed_e9_chain_demo.py`)."

    - agent: "main"
      message: "(SESI SEBELUMNYA — FASE D, disimpan sebagai riwayat) FASE D (Permintaan Desain / DSR + peran ke-7 designer) SIAP DIUJI LEWAT LAYAR. Keadaan terukur: gate.sh --full HIJAU 2x berturut (309s & 311s, 145 PASS, 0 FAIL) - POC FASE D 55/55 (4x, nol residu) - verify_data_integrity 237 PASS/0 FAIL/0 WARN - build FE bersih. SAYA SUDAH MENJALANKAN SENDIRI D-US1..D-US4 DI PERAMBAN (bukti: DSR-00014 dibuat & ditugaskan ke Sari Melati tenggat 2026-08-27; DSR-00003 diminta revisi ber-alasan lalu diserahkan ulang; Rapor Desainer & Rapor Saya terisi) dan data demo SUDAH DIPULIHKAN ke 4 dokumen. CARA MASUK: login-email-input / login-password-input / login-submit-button, sandi `demo12345`. AKUN BARU: designer@kainnusantara.id (Sari Melati, peran ke-7). SESUDAH masuk sebagai admin/manajer WAJIB memilih badan usaha: klik pita `scope-pick-ent_ksc` - mode 'Semua Entitas' sengaja HANYA-LIHAT dan menolak tulis dengan 409. JALAN KE LAYAR: navigasi BUKAN hash-routing; klik menu samping `nav-designer-hub` lalu tab hub `hub-tab-design-requests`. TESTID PENTING: design-requests-view, dsr-board, dsr-kpi-total/open/delivered/revision/overdue, dsr-create-button, dsr-create-submit, dsr-brief-input, dsr-due-input, dsr-assign-select, dsr-assign-due, dsr-assign-button, dsr-start-button, dsr-gallery-select, dsr-deliver-button, dsr-approve-button, dsr-reject-button, dsr-detail-reject-reason, dsr-detail-history, dsr-tab-board/list/report/myreport, designer-report-panel, my-designer-report-panel. Pop-up ALASAN memakai confirm-modal: isi `confirm-modal-reason` lalu `confirm-modal-confirm` (tombol itu DISABLED selama alasan kosong - itu perilaku yang DISENGAJA, bukan bug). YANG HARUS TIDAK ADA untuk peran designer: dsr-create-button, dsr-tab-report, dsr-approve-button, dsr-reject-button, dsr-assign-select (pemisahan tugas - jangan laporkan sebagai bug). JANGAN uji: drag-and-drop, unggah berkas fisik/kamera, suara. JANGAN laporkan 'NaN' dari pencarian case-insensitive: kata Indonesia ber-'nan' (mis. 'Penanganan') sering tertangkap - sesi lalu dua temuan seperti itu terbukti PALSU. WAJIB: reproduksi dulu sebelum melaporkan, dan periksa residu dokumen yang Anda buat (pemulihan: python seed_realistic.py lalu python seed_e9_chain_demo.py)."
